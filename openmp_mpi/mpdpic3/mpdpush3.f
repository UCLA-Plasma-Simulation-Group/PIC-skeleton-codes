c Fortran Library for Skeleton 3D Darwin MPI/OpenMP PIC Code
...
c-----------------------------------------------------------------------
     ...
     1ek,idimp,nppmx,nx,ny,nz,mx,my,mz,nxv,nypmx,nzpmx,mx1,myp1,mxyzp1, 
     2idds,ipbc)
...
c-----------------------------------------------------------------------
     ...
     1qbm,dt,dtc,ek,idimp,nppmx,nx,ny,nz,mx,my,mz,nxv,nypmx,nzpmx,mx1,  
     2myp1,mxyzp1,ntmax,idds,irc)
c for 3d code, this subroutine updates particle co-ordinates and
c velocities using leap-frog scheme in time and first-order linear
c interpolation in space, with magnetic field.  Using the Boris Mover.
...
c-----------------------------------------------------------------------
     ...
     1,nz,mx,my,mz,nxv,nypmx,nzpmx,mx1,myp1,mxyzp1,idds,ipbc)
...
c ppart(5,n,m) = velocity vy of particle n in partition in tile m
c ppart(6,n,m) = velocity vz of particle n in partition in tile m
c cu(i,j,k,l) = ith component of current density at grid point j,kk,ll
c where kk = k + noff(1) - 1, and ll = l + noff(2) - 1
c kpic = number of particles per tile
c noff(1) = lowermost global gridpoint in y in particle partition
c noff(2) = backmost global gridpoint in z in particle partition
...
!$OMP ATOMIC
     ...
     1      cu(1,i+noffp,mm+moffp,lm+loffp) + scu(1,i,mm,lm)
!$OMP ATOMIC
            cu(2,i+noffp,mm+moffp,lm+loffp) =                           
     1      cu(2,i+noffp,mm+moffp,lm+loffp) + scu(2,i,mm,lm)
!$OMP ATOMIC
            cu(3,i+noffp,mm+moffp,lm+loffp) =                           
     1      cu(3,i+noffp,mm+moffp,lm+loffp) + scu(3,i,mm,lm)
         endif
  130    continue
         do 140 j = 1, mm
!$OMP ATOMIC
...
!$OMP ATOMIC
     ...
     1      cu(1,nm+noffp,j+moffp,lm+loffp) + scu(1,nm,j,lm)
!$OMP ATOMIC
            cu(2,nm+noffp,j+moffp,lm+loffp) =                           
     1      cu(2,nm+noffp,j+moffp,lm+loffp) + scu(2,nm,j,lm)
!$OMP ATOMIC
            cu(3,nm+noffp,j+moffp,lm+loffp) =                           
     1      cu(3,nm+noffp,j+moffp,lm+loffp) + scu(3,nm,j,lm)
         endif
  140    continue
      endif
  150 continue
...
c-----------------------------------------------------------------------
     ...
     1mz,nxv,nypmx,nzpmx,mx1,myp1,mxyzp1,idds)
...
c ppart(1,n,m) = position x of particle n in partition in tile m at t
c ppart(2,n,m) = position y of particle n in partition in tile m at t
c ppart(3,n,m) = position z of particle n in partition in tile m at t
c ppart(4,n,m) = velocity vx of particle n in partition in tile m
c at t - dt/2
c ppart(5,n,m) = velocity vy of particle n in partition in tile m
c at t - dt/2
...
!$OMP ATOMIC
     ...
     1   amu(1,i+noffp,j+moffp,lm+loffp) + samu(1,i,j,lm)
!$OMP ATOMIC
         amu(2,i+noffp,j+moffp,lm+loffp) =
     1   amu(2,i+noffp,j+moffp,lm+loffp) + samu(2,i,j,lm)
!$OMP ATOMIC
         amu(3,i+noffp,j+moffp,lm+loffp) =
     1   amu(3,i+noffp,j+moffp,lm+loffp) + samu(3,i,j,lm)
!$OMP ATOMIC
         amu(4,i+noffp,j+moffp,lm+loffp) =
     1   amu(4,i+noffp,j+moffp,lm+loffp) + samu(4,i,j,lm)
!$OMP ATOMIC
         amu(5,i+noffp,j+moffp,lm+loffp) =
     1   amu(5,i+noffp,j+moffp,lm+loffp) + samu(5,i,j,lm)
!$OMP ATOMIC
         amu(6,i+noffp,j+moffp,lm+loffp) =
     1   amu(6,i+noffp,j+moffp,lm+loffp) + samu(6,i,j,lm)
      endif
   80 continue
   90 continue
      nm = min(mx+1,nxv-noffp)
...
!$OMP ATOMIC
     ...
     1   amu(1,i+noffp,mm+moffp,k+loffp) + samu(1,i,mm,k)
!$OMP ATOMIC
         amu(2,i+noffp,mm+moffp,k+loffp) =
     1   amu(2,i+noffp,mm+moffp,k+loffp) + samu(2,i,mm,k)
!$OMP ATOMIC
         amu(3,i+noffp,mm+moffp,k+loffp) =
     1   amu(3,i+noffp,mm+moffp,k+loffp) + samu(3,i,mm,k)
!$OMP ATOMIC
         amu(4,i+noffp,mm+moffp,k+loffp) =
     1   amu(4,i+noffp,mm+moffp,k+loffp) + samu(4,i,mm,k)
!$OMP ATOMIC
         amu(5,i+noffp,mm+moffp,k+loffp) =
     1   amu(5,i+noffp,mm+moffp,k+loffp) + samu(5,i,mm,k)
!$OMP ATOMIC
         amu(6,i+noffp,mm+moffp,k+loffp) =
     1   amu(6,i+noffp,mm+moffp,k+loffp) + samu(6,i,mm,k)
      endif
  100 continue
      do 110 j = 1, mm
!$OMP ATOMIC
...
!$OMP ATOMIC
     ...
     1   amu(1,nm+noffp,j+moffp,k+loffp) + samu(1,nm,j,k)
!$OMP ATOMIC
         amu(2,nm+noffp,j+moffp,k+loffp) =
     1   amu(2,nm+noffp,j+moffp,k+loffp) + samu(2,nm,j,k)
!$OMP ATOMIC
         amu(3,nm+noffp,j+moffp,k+loffp) =
     1   amu(3,nm+noffp,j+moffp,k+loffp) + samu(3,nm,j,k)
!$OMP ATOMIC
         amu(4,nm+noffp,j+moffp,k+loffp) =
     1   amu(4,nm+noffp,j+moffp,k+loffp) + samu(4,nm,j,k)
!$OMP ATOMIC
         amu(5,nm+noffp,j+moffp,k+loffp) =
     1   amu(5,nm+noffp,j+moffp,k+loffp) + samu(5,nm,j,k)
!$OMP ATOMIC
         amu(6,nm+noffp,j+moffp,k+loffp) =
     1   amu(6,nm+noffp,j+moffp,k+loffp) + samu(6,nm,j,k)
      endif
  110 continue
  120 continue
      if (lm > mz) then
...
!$OMP ATOMIC
     ...
     1   amu(1,i+noffp,1+moffp,lm+loffp) + samu(1,i,1,lm)
!$OMP ATOMIC
         amu(2,i+noffp,1+moffp,lm+loffp) =
     1   amu(2,i+noffp,1+moffp,lm+loffp) + samu(2,i,1,lm)
!$OMP ATOMIC
         amu(3,i+noffp,1+moffp,lm+loffp) =
     1   amu(3,i+noffp,1+moffp,lm+loffp) + samu(3,i,1,lm)
!$OMP ATOMIC
         amu(4,i+noffp,1+moffp,lm+loffp) =
     1   amu(4,i+noffp,1+moffp,lm+loffp) + samu(4,i,1,lm)
!$OMP ATOMIC
         amu(5,i+noffp,1+moffp,lm+loffp) =
     1   amu(5,i+noffp,1+moffp,lm+loffp) + samu(5,i,1,lm)
!$OMP ATOMIC
         amu(6,i+noffp,1+moffp,lm+loffp) =
     1   amu(6,i+noffp,1+moffp,lm+loffp) + samu(6,i,1,lm)
         if (mm > my) then
!$OMP ATOMIC
            amu(1,i+noffp,mm+moffp,lm+loffp) =
     1      amu(1,i+noffp,mm+moffp,lm+loffp) + samu(1,i,mm,lm)
!$OMP ATOMIC
            amu(2,i+noffp,mm+moffp,lm+loffp) =
     1      amu(2,i+noffp,mm+moffp,lm+loffp) + samu(2,i,mm,lm)
!$OMP ATOMIC
            amu(3,i+noffp,mm+moffp,lm+loffp) =
     1      amu(3,i+noffp,mm+moffp,lm+loffp) + samu(3,i,mm,lm)
!$OMP ATOMIC
            amu(4,i+noffp,mm+moffp,lm+loffp) =
     1      amu(4,i+noffp,mm+moffp,lm+loffp) + samu(4,i,mm,lm)
!$OMP ATOMIC
            amu(5,i+noffp,mm+moffp,lm+loffp) =
     1      amu(5,i+noffp,mm+moffp,lm+loffp) + samu(5,i,mm,lm)
!$OMP ATOMIC
            amu(6,i+noffp,mm+moffp,lm+loffp) =
     1      amu(6,i+noffp,mm+moffp,lm+loffp) + samu(6,i,mm,lm)
         endif
  130    continue
         do 140 j = 1, mm
!$OMP ATOMIC
         amu(1,1+noffp,j+moffp,lm+loffp) =
     1   amu(1,1+noffp,j+moffp,lm+loffp) + samu(1,1,j,lm)
!$OMP ATOMIC
         amu(2,1+noffp,j+moffp,lm+loffp) =
     1   amu(2,1+noffp,j+moffp,lm+loffp) + samu(2,1,j,lm)
!$OMP ATOMIC
         amu(3,1+noffp,j+moffp,lm+loffp) =
     1   amu(3,1+noffp,j+moffp,lm+loffp) + samu(3,1,j,lm)
!$OMP ATOMIC
         amu(4,1+noffp,j+moffp,lm+loffp) =
     1   amu(4,1+noffp,j+moffp,lm+loffp) + samu(4,1,j,lm)
!$OMP ATOMIC
         amu(5,1+noffp,j+moffp,lm+loffp) =
     1   amu(5,1+noffp,j+moffp,lm+loffp) + samu(5,1,j,lm)
!$OMP ATOMIC
         amu(6,1+noffp,j+moffp,lm+loffp) =
     1   amu(6,1+noffp,j+moffp,lm+loffp) + samu(6,1,j,lm)
         if (nm > mx) then
!$OMP ATOMIC
            amu(1,nm+noffp,j+moffp,lm+loffp) =
     1      amu(1,nm+noffp,j+moffp,lm+loffp) + samu(1,nm,j,lm)
!$OMP ATOMIC
            amu(2,nm+noffp,j+moffp,lm+loffp) =
     1      amu(2,nm+noffp,j+moffp,lm+loffp) + samu(2,nm,j,lm)
!$OMP ATOMIC
            amu(3,nm+noffp,j+moffp,lm+loffp) =
     1      amu(3,nm+noffp,j+moffp,lm+loffp) + samu(3,nm,j,lm)
!$OMP ATOMIC
            amu(4,nm+noffp,j+moffp,lm+loffp) =
     1      amu(4,nm+noffp,j+moffp,lm+loffp) + samu(4,nm,j,lm)
!$OMP ATOMIC
            amu(5,nm+noffp,j+moffp,lm+loffp) =
     1      amu(5,nm+noffp,j+moffp,lm+loffp) + samu(5,nm,j,lm)
!$OMP ATOMIC
            amu(6,nm+noffp,j+moffp,lm+loffp) =
     1      amu(6,nm+noffp,j+moffp,lm+loffp) + samu(6,nm,j,lm)
         endif
  140    continue
      endif
  150 continue
...
c-----------------------------------------------------------------------
     ...
     1,qbm,dt,idimp,nppmx,nx,mx,my,mz,nxv,nypmx,nzpmx,mx1,myp1,mxyzp1,  
     2idds)
c for 3 code, this subroutine calculates particle momentum flux and
c acceleration density using first-order spline interpolation.
c for distributed data, with 2D spatial decomposition
c OpenMP version using guard cells
...
c at t - dt/2
c fxyz(1,j,k,l) = x component of force/charge at grid (j,kk,ll)
c fxyz(2,j,k,l) = y component of force/charge at grid (j,kk,lll)
c fxyz(3,j,k,l) = z component of force/charge at grid (j,kk,ll)
c that is, convolution of electric field over particle shape
c bxyz(1,j,k,l) = x component of magnetic field at grid (j,kk,ll)
c bxyz(2,j,k,l) = y component of magnetic field at grid (j,kk,ll)
...
!$OMP ATOMIC
     ...
     1   dcu(1,i+noffp,j+moffp,lm+loffp) + sdcu(1,i,j,lm)
!$OMP ATOMIC
         dcu(2,i+noffp,j+moffp,lm+loffp) =
     1   dcu(2,i+noffp,j+moffp,lm+loffp) + sdcu(2,i,j,lm)
!$OMP ATOMIC
         dcu(3,i+noffp,j+moffp,lm+loffp) =
     1   dcu(3,i+noffp,j+moffp,lm+loffp) + sdcu(3,i,j,lm)
      endif
  140 continue
  150 continue
      nm = min(mx+1,nxv-noffp)
...
!$OMP ATOMIC
     ...
     1   dcu(1,i+noffp,mm+moffp,k+loffp) + sdcu(1,i,mm,k)
!$OMP ATOMIC
         dcu(2,i+noffp,mm+moffp,k+loffp) =
     1   dcu(2,i+noffp,mm+moffp,k+loffp) + sdcu(2,i,mm,k)
!$OMP ATOMIC
         dcu(3,i+noffp,mm+moffp,k+loffp) =
     1   dcu(3,i+noffp,mm+moffp,k+loffp) + sdcu(3,i,mm,k)
      endif
  160 continue
      do 170 j = 1, mm
!$OMP ATOMIC
...
!$OMP ATOMIC
     ...
     1   dcu(1,nm+noffp,j+moffp,k+loffp) + sdcu(1,nm,j,k)
!$OMP ATOMIC
         dcu(2,nm+noffp,j+moffp,k+loffp) =
     1   dcu(2,nm+noffp,j+moffp,k+loffp) + sdcu(2,nm,j,k)
!$OMP ATOMIC
         dcu(3,nm+noffp,j+moffp,k+loffp) =
     1   dcu(3,nm+noffp,j+moffp,k+loffp) + sdcu(3,nm,j,k)
      endif
  170 continue
  180 continue
      if (lm > mz) then
...
!$OMP ATOMIC
     ...
     1   dcu(1,i+noffp,1+moffp,lm+loffp) + sdcu(1,i,1,lm)
!$OMP ATOMIC
         dcu(2,i+noffp,1+moffp,lm+loffp) =
     1   dcu(2,i+noffp,1+moffp,lm+loffp) + sdcu(2,i,1,lm)
!$OMP ATOMIC
         dcu(3,i+noffp,1+moffp,lm+loffp) =
     1   dcu(3,i+noffp,1+moffp,lm+loffp) + sdcu(3,i,1,lm)
         if (mm > my) then
!$OMP ATOMIC
            amu(1,i+noffp,mm+moffp,lm+loffp) =
     1      amu(1,i+noffp,mm+moffp,lm+loffp) + samu(1,i,mm,lm)
...
!$OMP ATOMIC
     ...
     1      dcu(1,i+noffp,mm+moffp,lm+loffp) + sdcu(1,i,mm,lm)
!$OMP ATOMIC
            dcu(2,i+noffp,mm+moffp,lm+loffp) =
     1      dcu(2,i+noffp,mm+moffp,lm+loffp) + sdcu(2,i,mm,lm)
!$OMP ATOMIC
            dcu(3,i+noffp,mm+moffp,lm+loffp) =  
     1      dcu(3,i+noffp,mm+moffp,lm+loffp) + sdcu(3,i,mm,lm)
         endif
  190    continue
         do 200 j = 1, mm
!$OMP ATOMIC
...
!$OMP ATOMIC
     ...
     1   dcu(1,1+noffp,j+moffp,lm+loffp) + sdcu(1,1,j,lm)
!$OMP ATOMIC
         dcu(2,1+noffp,j+moffp,lm+loffp) =
     1   dcu(2,1+noffp,j+moffp,lm+loffp)  + sdcu(2,1,j,lm)
!$OMP ATOMIC
         dcu(3,1+noffp,j+moffp,lm+loffp) =
     1   dcu(3,1+noffp,j+moffp,lm+loffp) + sdcu(3,1,j,lm)
         if (nm > mx) then
!$OMP ATOMIC
            amu(1,nm+noffp,j+moffp,lm+loffp) =
     1      amu(1,nm+noffp,j+moffp,lm+loffp) + samu(1,nm,j,lm)
...
!$OMP ATOMIC
     ...
     1      dcu(1,nm+noffp,j+moffp,lm+loffp) + sdcu(1,nm,j,lm)
!$OMP ATOMIC
            dcu(2,nm+noffp,j+moffp,lm+loffp) =
     1      dcu(2,nm+noffp,j+moffp,lm+loffp) + sdcu(2,nm,j,lm)
!$OMP ATOMIC
            dcu(3,nm+noffp,j+moffp,lm+loffp) =
     1      dcu(3,nm+noffp,j+moffp,lm+loffp) + sdcu(3,nm,j,lm)
         endif
  200    continue
      endif
  210 continue
...
c-----------------------------------------------------------------------
     ...
     1amu,qm,qbm,dt,idimp,nppmx,nx,mx,my,mz,nxv,nypmx,nzpmx,mx1,myp1,   
     2mxyzp1,idds)
...
c bxyz(1,l,j,k) = x component of complex magnetic field
c bxyz(2,l,j,k) = y component of complex magnetic field
c bxyz(3,l,j,k) = z component of complex magnetic field
c all for fourier mode jj-1,kk-1,l-1, where jj = j + kxyp*js and
c kk = k + kyzp*ks, and MPI rank idproc = js + nvpy*ks
c aimag(ffc(l,j,k)) = finite-size particle shape factor s
c real(ffc(l,j,k)) = potential green's function g
...
c for distributed data with 2D decomposition and OpenMP
...
c                      +kz*dcu(3,kx,ky,kz))/(kx*kx+ky*ky+kz*kz)
c on output:
c dcu(i,l,j,k) = transverse part of complex derivative of current for
c all for fourier mode jj-1,kk-1,l-1, where jj = j + kxyp*js and
c kk = k + kyzp*ks, and MPI rank idproc = js + nvpy*ks
c amu(1,l,j,k) = xx component of complex momentum flux
c amu(2,l,j,k) = xy component of complex momentum flux
c amu(3,l,j,k) = xz component of complex momentum flux
c amu(4,l,j,k) = yy component of complex momentum flux
c amu(5,l,j,k) = yz component of complex momentum flux
c amu(6,l,j,k) = zz component of complex momentum flux
c all for fourier mode jj-1,kk-1,l-1, where jj = j + kxyp*js and
c kk = k + kyzp*ks, and MPI rank idproc = js + nvpy*ks
c nx/ny/nz = system length in x/y/z direction
c kstrt = starting data block number
c nvpy/nvpz = number of procs in y/z
c nzv = second dimension of field arrays, must be >= nz
c kxyp/kyzp = number of complex grids in each field partition in
c x/y direction
      implicit none
      integer nx, ny, nz, kstrt, nvpy, nvpz, nzv, kxyp, kyzp
...
c                      +kz*dcu(3,kx,ky,kz))/(kx*kx+ky*ky+kz*kz)
c on input:
c dcu(i,l,j,k) = complex acceleration density for fourier mode (j-1,k-1)
c on output:
c dcu(i,l,j,k) = transverse part of complex derivative of current for
c all for fourier mode jj-1,kk-1,l-1, where jj = j + kxyp*js and
c kk = k + kyzp*ks, and MPI rank idproc = js + nvpy*ks
...
c-----------------------------------------------------------------------
     ...
     1,nx,ny,nz,kstrt,nvpy,nvpz,nzv,kxyp,kyzp,nzhd)
c this subroutine solves 3d poisson's equation in fourier space for
...
c input: isign,ax,ay,az,affp,wp0,nx,ny,nz,kstrt,nvpy,nvpz,kxyp,kyzp,nzhd
c for isign /= 0, output: exyz,wf
c input: dcu,ffe,isign,affp,ci,nx,ny,nz,kstrt,nzv,kxyp,kyzp,nzhd
c approximate flop count is:
...
c dcu(l,j,k) = transverse part of complex derivative of current for
c fourier mode jj-1,kk-1,l-1
c exyz(1,l,j,k) = x component of complex transverse electric field
c exyz(2,l,j,k) = y component of complex transverse electric field
c exyz(3,l,j,k) = z component of complex transverse electric field
c all for fourier mode jj-1,kk-1,l-1, where jj = j + kxyp*js and
c kk = k + kyzp*ks, and MPI rank idproc = js + nvpy*ks
c aimag(ffe(l,j,k)) = finite-size particle shape factor s
c real(ffe(l,j,k)) = potential green's function g
c all for fourier mode jj-1,kk-1,l-1, where jj = j + kxyp*js and
c kk = k + kyzp*ks, and MPI rank idproc = js + nvpy*ks
c ax/ay/az = half-width of particle in x/y/z direction
c affp = normalization constant = nx*ny*nz/np,
...
!$OMP DO PRIVATE(j,k,l,kk,k1,l1,at1,at2,wp) REDUCTION(+:sum1)
     ...
     1              + dcu(2,l,j,k)*conjg(dcu(2,l,j,k))
     2              + dcu(3,l,j,k)*conjg(dcu(3,l,j,k))
     3              + dcu(1,l1,j,k)*conjg(dcu(1,l1,j,k))
     4              + dcu(2,l1,j,k)*conjg(dcu(2,l1,j,k))
     5              + dcu(3,l1,j,k)*conjg(dcu(3,l1,j,k)))
   50       continue
c mode numbers kz = 0, nz/2
            l1 = nzh + 1
     ...
     1              + dcu(2,1,j,k)*conjg(dcu(2,1,j,k))
     2              + dcu(3,1,j,k)*conjg(dcu(3,1,j,k)))
         endif
      endif
      sum1 = sum1 + wp
   60 continue
...
c keep kx = 0
     ...
     1                 + dcu(2,l,1,k)*conjg(dcu(2,l,1,k))
     2                 + dcu(3,l,1,k)*conjg(dcu(3,l,1,k))
     3                 + dcu(1,l1,1,k)*conjg(dcu(1,l1,1,k))
     4                 + dcu(2,l1,1,k)*conjg(dcu(2,l1,1,k))
     5                 + dcu(3,l1,1,k)*conjg(dcu(3,l1,1,k)))
   70          continue
c mode numbers kz = 0, nz/2
               l1 = nzh + 1
     ...
     1                 + dcu(2,1,1,k)*conjg(dcu(2,1,1,k))
     2                 + dcu(3,1,1,k)*conjg(dcu(3,1,1,k)))
c throw away kx = nx/2
            else
               do 80 l = 1, nz
               exyz(1,l,1,k) = zero
...
!$OMP PARALLEL DO PRIVATE(j,l,l1,at1,at2,wp) REDUCTION(+:sum2)
     ...
     1              + dcu(2,l,j,1)*conjg(dcu(2,l,j,1))
...
c mode numbers kz = 0, nz/2
     ...
     1              + dcu(2,1,j,1)*conjg(dcu(2,1,j,1))
     2              + dcu(3,1,j,1)*conjg(dcu(3,1,j,1)))
         endif
         sum2 = sum2 + wp
  110    continue
!$OMP END PARALLEL DO
...
c mode numbers kx = 0, nx/2
     ...
     1              + dcu(2,l,1,1)*conjg(dcu(2,l,1,1))
     2              + dcu(3,l,1,1)*conjg(dcu(3,l,1,1)))
  120       continue
c mode numbers kz = 0, nz/2
            l1 = nzh + 1
            exyz(1,1,1,1) = zero
...
c with 2D decomposition and OpenMP
      implicit none
...
c-----------------------------------------------------------------------
     ...
     1indx,indy,indz,kstrt,nvpy,nvpz,nxvh,nyv,nzv,kxyp,kyp,kyzp,kzp,
     2kxypd,kypd,kyzpd,kzpd,kzyp,ndim,nxhyzd,nxyzhd)
c wrapper function for multiple 3d real to complex ffts,
c with packed data, parallelized with MPI/OpenMP
      implicit none
      integer isign, ntpose, indx, indy, indz, kstrt, nvpy, nvpz
      integer nxvh, nyv, nzv, kxyp, kyp, kyzp, kzp
...
c perform x fft
     ...
     1,kypi,kypp,nxvh,kzpp,kypd,kzpd,ndim,nxhyzd,nxyzhd)
c transpose f array to g
         call PWTIMERA(-1,ttp,dtime)
...
c perform y fft
     ...
     1nvpz,kxypi,kxypp,nyv,kzpp,kxypd,kzpd,ndim,nxhyzd,nxyzhd)
c transpose g array to h
         call PWTIMERA(-1,tp,dtime)
         call PPNTPOS3B(g,h,bs,br,nxh,ny,nz,kxyp,kyzp,kzp,kstrt,nvpy,   
...
c perform z fft
     ...
     1nvpz,kxypi,kxypp,nzv,kyzp,kxypd,kyzpd,ndim,nxhyzd,nxyzhd)
c transpose h array to f
         if (ntpose.eq.0) then
            call PWTIMERA(-1,tf,dtime)
            call PPNTPOS3B(h,g,br,bs,nxh,nz,ny,kxyp,kzp,kyzp,kstrt,nvpy,
     1nvpz,ndim,nzv,nyv,kxypd,kzpd,kyzpd)
            call PPNTPOS3A(g,f,br,bs,ny,nxh,nz,kyp,kxyp,kzp,kstrt,nvpy, 
     1ndim,nyv,nxvh,kypd,kxypd,kzpd)
            call PWTIMERA(1,tf,dtime)
         endif
c forward fourier transform
      else if (isign.gt.0) then
c transpose f array to h
     ...
     1ndim,nxvh,nyv,kxypd,kypd,kzpd)
            call PPNTPOS3B(g,h,bs,br,nxh,ny,nz,kxyp,kyzp,kzp,kstrt,nvpy,
     1nvpz,ndim,nyv,nzv,kxypd,kyzpd,kzpd)
            call PWTIMERA(1,tf,dtime)
         endif
...
c-----------------------------------------------------------------------
     ...
     1nvp,kypi,kypp,nxvh,kzpp,kypd,kzpd,ndim,nxhyzd,nxyzhd)
c this subroutine performs the x part of N three dimensional real to
c complex fast fourier transforms and their inverses for a subset of
c y and z, using complex arithmetic, where N = ndim, with OpenMP
c for data which is distributed in blocks, with 2D spatial decomposition
c for isign = (-1,1), input: all, output: f
c for isign = -1, approximate flop count: M*(5*log2(M) + 19/2)/nvp
c for isign = 1,  approximate flop count: M*(5*log2(M) + 15/2)/nvp
c where M = (nx/2)*ny*nz, and nvp = number of procs
c indx/indy/indz = exponent which determines length in x/y/z direction,
c where nx=2**indx, ny=2**indy, nz=2**indz
c if isign = -1, an inverse fourier transform is performed
c f(1:N,n,k,i) = (1/nx*ny*nz)*sum(f(1:N,j,k,i)*
c                                 exp(-sqrt(-1)*2pi*n*j/nx))
c if isign = 1, a forward fourier transform is performed
c f(1:N,n,k,i) = sum(f(1:N,j,k,i)*exp(sqrt(-1)*2pi*n*j/nx))
c kstrt = starting data block number
c nvp = number of real or virtual processors
c kypi = initial y index used
c kypp = number of y indices used
...
c with the odd/even x points stored in the real/imaginary parts.
c final fourier coefficients are stored as follows:
c h(1:N,l,j,k) = mode jj-1,kk-1,l, where jj = j + kxyp*js,
c kk = k + kyzp*ks, and MPI rank idproc = js + nvpy*ks
c 1 <= jj <= nx/2, 1 <= kk <= ny, and 1 <= l <= nz, except for
c h(1:N,l,1,k) = mode nx/2,kk-1,l-1,
c where ny/2+2 <= kk <= ny, 1 <= l <= nz,
c the following are located on node js = 0 and ks = 0:
c h(1:N,l,1,1) = mode nx/2,0,l-1, where 2 <= l <= nz/2
c imag(h(1:N,1,1,1)) = real part of mode nx/2,0,0
c imag(h(1:N,nz/2+1,1,1)) = real part of mode nx/2,0,nz/2
c the following are located on node js = 0 and ks = nyh/kyzp:
c h(1:N,l,1,ny/2+1) = mode nx/2,ny/2,l-1, where nz/2+2 <= l <= nz, and
c imag(h(1:N,1,1,ny/2+1)) = real part of mode nx/2,ny/2,0
c imag(h(1:N,nz/2+1,1,ny/2+1)) = real part of mode nx/2,ny/2,nz/2
c using jpl storage convention, as described in:
c E. Huang, P. C. Liewer, V. K. Decyk, and R. D. Ferraro, "Concurrent
c Three-Dimensional Fast Fourier Transform Algorithms for Coarse-Grained
c Distributed Memory Parallel Computers," Caltech CRPC Report 217-50,
...
c-----------------------------------------------------------------------
     ...
     1nvpy,nvpz,kxypi,kxypp,nyv,kzpp,kxypd,kzpd,ndim,nxhyzd,nxyzhd)
c this subroutine performs the y part of N three dimensional real to
c complex fast fourier transforms and their inverses for a subset of
c x and z, using complex arithmetic, where N = ndim, with OpenMP
c for data which is distributed in blocks, with 2D spatial decomposition
c for isign = (-1,1), input: all, output: g
c for isign = -1, approximate flop count: M*(5*log2(M) + 19/2)/nvp
c for isign = 1,  approximate flop count: M*(5*log2(M) + 15/2)/nvp
...
c where nx=2**indx, ny=2**indy, nz=2**indz
c if isign = -1, an inverse fourier transform is performed
c g(1:N,m,j,i) = sum(g(1:N,k,j,i)*exp(-sqrt(-1)*2pi*m*k/ny))
c if isign = 1, a forward fourier transform is performed
c g(1:N,m,j,i) = sum(g(1:N,k,j,i)*exp(sqrt(-1)*2pi*m*k/ny))
c kstrt = starting data block number
c nvpy/nvpz = number of real or virtual processors in y/z
...
c-----------------------------------------------------------------------
     ...
     1nvpy,nvpz,kxypi,kxypp,nzv,kyzp,kxypd,kyzpd,ndim,nxhyzd,nxyzhd)
c this subroutine performs the z part of N three dimensional real to
c complex fast fourier transforms and their inverses for a subset of
c x and y, using complex arithmetic, where N = ndim, with OpenMP
c for data which is distributed in blocks, with 2D spatial decomposition
c for isign = (-1,1), input: all, output: h
c for isign = -1, approximate flop count: M*(5*log2(M) + 19/2)/nvp
c for isign = 1,  approximate flop count: M*(5*log2(M) + 15/2)/nvp
...
c where nx=2**indx, ny=2**indy, nz=2**indz
c if isign = -1, an inverse fourier transform is performed
c h(1:N,l,n,m) = sum(h(1:N,i,j,k)*exp(-sqrt(-1)*2pi*l*i/nz))
c if isign = 1, a forward fourier transform is performed
c h(1:N,l,n,m) = sum(h(1:N,i,j,k)*exp(sqrt(-1)*2pi*ll*ii/nz))
c kstrt = starting data block number
c nvpy/nvpz = number of real or virtual processors in y/z
c kxypi = initial x index used
c kxypp = number of x indices used
...
c kzpp = number of z indices used
c kypd, kzpd = third and fourth dimension of f
c ndim = leading dimension of array f
      implicit none
