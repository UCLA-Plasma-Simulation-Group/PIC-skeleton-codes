...
isign = 0
mppois22(qt,fxyt,isign,ffc,ax,ay,affp,we,nx,ny,kstrt,nye,kxp,nyh)
# initialize electrons
nps = 1
nnpp[0] = 0
pdistr2(part,edges,nnpp,nps,vtx,vty,vx0,vy0,npx,npy,nx,ny,idimp,npmax,
        idps,ipbc,ierr)
npp = nnpp[0]
...
# transform force to real space with OpenMP: updates fxye, modifies fxyt
   dtimer(dtime,itime,-1)
   isign = 1
   wppfft2rm2(fxye,fxyt,bs,br,isign,ntpose,mixup,sct,ttp,indx,indy,
...
# copy guard cells with OpenMP: updates fxye
   dtimer(dtime,itime,-1)
   ppncguard2l(fxye,nyp,kstrt,nvp,nnxe,nypmx)
   ppcguard2xl(fxye,nyp,nx,ndim,nxe,nypmx)
   dtimer(dtime,itime,1)
   time = float(dtime)
   tguard = tguard + time
...
#  ppgppush2l(ppart,fxye,kpic,noff,nyp,qbme,dt,wke,nx,ny,mx,my,idimp,
#             nppmx0,nxe,nypmx,mx1,mxyp1,ipbc)
# updates ppart, wke, ncl, iholep, irc
   ppgppushf2l(ppart,fxye,kpic,ncl,iholep,noff,nyp,qbme,dt,wke,nx,ny,mx,
