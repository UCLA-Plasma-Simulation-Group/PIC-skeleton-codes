...
c-----------------------------------------------------------------------
     ...
     1,ci,ek,idimp,nppmx,nx,ny,nz,mx,my,mz,nxv,nypmx,nzpmx,mx1,myp1,    
     2mxyzp1,idds,ipbc)
c for 3d code, this subroutine updates particle co-ordinates and
c velocities using leap-frog scheme in time and first-order linear
...
c ppart(2,n,m) = position y of particle n in partition in tile m
c ppart(3,n,m) = position z of particle n in partition in tile m
c ppart(4,n,m) = momentum px of particle n in partition in tile m
c ppart(5,n,m) = momentum py of particle n in partition in tile m
c ppart(6,n,m) = momentum pz of particle n in partition in tile m
c fxyz(1,j,k,l) = x component of force/charge at grid (j,kk,ll)
c fxyz(2,j,k,l) = y component of force/charge at grid (j,kk,ll)
c fxyz(3,j,k,l) = z component of force/charge at grid (j,kk,ll)
c that is, convolution of electric field over particle shape
...
c-----------------------------------------------------------------------
     ...
     1,qbm,dt,dtc,ci,ek,idimp,nppmx,nx,ny,nz,mx,my,mz,nxv,nypmx,nzpmx,  
     2mx1,myp1,mxyzp1,ntmax,idds,irc)
c for 3d code, this subroutine updates particle co-ordinates and
c velocities using leap-frog scheme in time and first-order linear
c interpolation in space, for relativistic particles with magnetic field
...
c-----------------------------------------------------------------------
     ...
     1nppmx,idimp,nx,ny,nz,mx,my,mz,nxv,nypmx,nzpmx,mx1,myp1,mxyzp1,    
     2ntmax,idds,irc)
c for 3d code, this subroutine calculates particle current density
c using first-order linear interpolation.
c in addition, particle positions are advanced a half time-step
...
c particles stored segmented array
c 69 flops/particle, 30 loads, 27 stores
c input: all except ncl, ihole, irc, output: ppart, cu, ncl, ihole, irc
c current density is approximated by values at the nearest grid points
c cu(i,n,m,l)=qci*(1.-dx)*(1.-dy)*(1.-dz)
c cu(i,n+1,m,l)=qci*dx*(1.-dy)*(1.-dz)
c cu(i,n,m+1,l)=qci*(1.-dx)*dy*(1.-dz)
...
c-----------------------------------------------------------------------
     ...
     1nx,ny,nz,mx,my,mz,nxv,nypmx,nzpmx,mx1,myp1,mxyzp1,idds,ipbc)
c for 3d code, this subroutine calculates particle current density
...
c ppart(2,n,m) = position y of particle n in partition in tile m
c ppart(3,n,m) = position z of particle n in partition in tile m
c ppart(4,n,m) = x momentum of particle n in partition in tile m
c ppart(5,n,m) = y momentum of particle n in partition in tile m
c ppart(6,n,m) = z momentum of particle n in partition in tile m
c cu(i,j,k,l) = ith component of current density at grid point j,kk,ll
c where kk = k + noff(1) - 1, and ll = l + noff(2) - 1
c kpic = number of particles per tile
c noff(1) = lowermost global gridpoint in y in particle partition
...
c-----------------------------------------------------------------------
     ...
     1ci,nppmx,idimp,nx,ny,nz,mx,my,mz,nxv,nypmx,nzpmx,mx1,myp1,mxyzp1, 
     2ntmax,idds,irc)
c for 3d code, this subroutine calculates particle current density
c using first-order linear interpolation for relativistic particles
c in addition, particle positions are advanced a half time-step
