...
# deposit current with OpenMP:
   ...
   if (relativity==1):
# updates ppart, cue
#     ppgrjppost2l(ppart,cue,kpic,noff,qme,dth,ci,nppmx0,idimp,nx,ny,mx,
#                  my,nxe,nypmx,mx1,mxyp1,ipbc)
# updates ppart, cue, ncl, iholep, irc
      ppgrjppostf2l(ppart,cue,kpic,ncl,iholep,noff,nyp,qme,dth,ci,
                    nppmx0,idimp,nx,ny,mx,my,nxe,nypmx,mx1,mxyp1,ntmaxp,
                    irc)
   else:
# updates ppart, cue
#     ppgjppost2l(ppart,cue,kpic,noff,qme,dth,nppmx0,idimp,nx,ny,mx,my,
#                 nxe,nypmx,mx1,mxyp1,ipbc)
# updates ppart, cue, ncl, iholep, irc
      ppgjppostf2l(ppart,cue,kpic,ncl,iholep,noff,nyp,qme,dth,nppmx0,
...
# updates exyz, bxyz, wf, wm
   dtimer(dtime,itime,-1)
   if (ntime==0):
      mippbpoisp23(cut,bxyz,ffc,ci,wm,nx,ny,kstrt,nye,kxp,nyh)
      wf[0] = 0.0
      dth = 0.5*dt
   else:
      mppmaxwel2(exyz,bxyz,cut,ffc,affp,ci,dt,wf,wm,nx,ny,kstrt,nye,kxp,
                 nyh)
   dtimer(dtime,itime,1)
   time = float(dtime)
   tfield = tfield + time
...
# updates fxyt
   dtimer(dtime,itime,-1)
   isign = 1
   mppemfield2(fxyt,exyz,ffc,isign,nx,ny,kstrt,nye,kxp,nyh)
# copy magnetic field with OpenMP: updates bxyt
   isign = -1
   mppemfield2(bxyt,bxyz,ffc,isign,nx,ny,kstrt,nye,kxp,nyh)
   dtimer(dtime,itime,1)
   time = float(dtime)
   tfield = tfield + time
...
#     ppgrbppush23l(ppart,fxyze,bxyze,kpic,noff,nyp,qbme,dt,dth,ci,wke,
#                   idimp,nppmx0,nx,ny,mx,my,nxe,nypmx,mx1,mxyp1,ipbc)
# updates ppart, ncl, iholep, ek, irc
      ppgrbppushf23l(ppart,fxyze,bxyze,kpic,ncl,iholep,noff,nyp,qbme,dt,
                     dth,ci,wke,idimp,nppmx0,nx,ny,mx,my,nxe,nypmx,mx1,
                     mxyp1,ntmaxp,irc)
   else:
# updates ppart and wke
#     ppgbppush23l(ppart,fxyze,bxyze,kpic,noff,nyp,qbme,dt,dth,wke,
#                  idimp,nppmx0,nx,ny,mx,my,nxe,nypmx,mx1,mxyp1,ipbc)
# updates ppart, ncl, iholep, ek, irc
      ppgbppushf23l(ppart,fxyze,bxyze,kpic,ncl,iholep,noff,nyp,qbme,dt,
