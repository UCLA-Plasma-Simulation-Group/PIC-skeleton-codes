...
tdjpost = 0.0; tpush = 0.0; tsort = 0.0; tmov = 0.0
...
exyz = numpy.empty((ndim,nye,kxp),complex_type,'F')
# bxyz = magnetic field in fourier space
bxyz = numpy.empty((ndim,nye,kxp),complex_type,'F')
...
nps = 1
nnpp[0] = 0
cpdistr2h(part,edges,nnpp,nps,vtx,vty,vtz,vx0,vy0,vz0,npx,npy,nx,ny,
...
# deposit current with OpenMP:
   ...
   if (relativity==1):
# updates ppart, cue
#     cppgrjppost2l(ppart,cue,kpic,noff,qme,dth,ci,nppmx0,idimp,nx,ny,mx,
#                   my,nxe,nypmx,mx1,mxyp1,ipbc)
# updates ppart, cue, ncl, iholep, irc
      cppgrjppostf2l(ppart,cue,kpic,ncl,iholep,noff,nyp,qme,dth,ci,
                     nppmx0,idimp,nx,ny,mx,my,nxe,nypmx,mx1,mxyp1,ntmaxp,
                     irc)
   else:
# updates ppart, cue
#     cppgjppost2l(ppart,cue,kpic,noff,qme,dth,nppmx0,idimp,nx,ny,mx,my,
#                  nxe,nypmx,mx1,mxyp1,ipbc)
# updates ppart, cue, ncl, iholep, irc
      cppgjppostf2l(ppart,cue,kpic,ncl,iholep,noff,nyp,qme,dth,nppmx0,
                    idimp,nx,ny,mx,my,nxe,nypmx,mx1,mxyp1,ntmaxp,irc)
   dtimer(dtime,itime,1)
   time = float(dtime)
...
# modifies cue
   dtimer(dtime,itime,-1)
   isign = -1
   cwppfft2rm3(cue,cut,bs,br,isign,ntpose,mixup,sct,ttp,indx,indy,kstrt,
...
# updates exyz, bxyz, wf, wm
   ...
   if (ntime==0):
      ...
      cmppmaxwel2(exyz,bxyz,cut,ffc,affp,ci,dt,wf,wm,nx,ny,kstrt,nye,kxp,
                  nyh)
   dtimer(dtime,itime,1)
   time = float(dtime)
   tfield = tfield + time
...
# modifies fxyt
   dtimer(dtime,itime,-1)
   isign = 1
   cwppfft2rm3(fxyze,fxyt,bs,br,isign,ntpose,mixup,sct,ttp,indx,indy,
...
# modifies bxyt
   dtimer(dtime,itime,-1)
   isign = 1
   cwppfft2rm3(bxyze,bxyt,bs,br,isign,ntpose,mixup,sct,ttp,indx,indy,
               kstrt,nvp,nxeh,nye,kxp,kyp,nypmx,nxhy,nxyh)
   dtimer(dtime,itime,1)
   time = float(dtime)
   tfft[0] = tfft[0] + time
...
#     cppgrbppush23l(ppart,fxyze,bxyze,kpic,noff,nyp,qbme,dt,dth,ci,wke,
#                    idimp,nppmx0,nx,ny,mx,my,nxe,nypmx,mx1,mxyp1,ipbc)
# updates ppart, ncl, iholep, ek, irc
      cppgrbppushf23l(ppart,fxyze,bxyze,kpic,ncl,iholep,noff,nyp,qbme,dt,
                      dth,ci,wke,idimp,nppmx0,nx,ny,mx,my,nxe,nypmx,mx1,
                      mxyp1,ntmaxp,irc)
   else:
# updates ppart and wke
...
#                   idimp,nppmx0,nx,ny,mx,my,nxe,nypmx,mx1,mxyp1,ipbc)
# updates ppart, ncl, iholep, ek, irc
      cppgbppushf23l(ppart,fxyze,bxyze,kpic,ncl,iholep,noff,nyp,qbme,dt,
                     dth,wke,idimp,nppmx0,nx,ny,mx,my,nxe,nypmx,mx1,
                     mxyp1,ntmaxp,irc)
   dtimer(dtime,itime,1)
   time = float(dtime)
   tpush = tpush + time
   if (irc[0] != 0):
      if (relativity==1):
         print "ppgrbppushf23l error, irc=", irc[0]
      else:
         print "ppgbppushf23l error, irc=", irc[0]
      cppabort()
      exit(0)
