...
isign = 0
cmppepoisp23(dcut,cur,isign,ffe,ax,ay,affp,wpm,ci,wf,nx,ny,kstrt,nye,kxp,
             nyh)
...
# deposit current with OpenMP: updates cue
   ...
   cue.fill(0.0)
   cppgjppost2l(ppart,cue,kpic,noff,qme,zero,nppmx0,idimp,nx,ny,mx,my,
...
# with OpenMP: updates dcut, amut, modifies dcu, amu
   dtimer(dtime,itime,-1)
   isign = -1
   cwppfft2rm3(dcu,dcut,bs,br,isign,ntpose,mixup,sct,ttp,indx,indy,kstrt,
               nvp,nxeh,nye,kxp,kyp,nypmx,nxhy,nxyh)
   tfft[1] = tfft[1] + ttp[0]
   cwppfft2rmn(amu,amut,bs,br,ss,isign,ntpose,mixup,sct,ttp,indx,indy,
...
# updates cur, wf
   dtimer(dtime,itime,-1)
   isign = -1
   cmppepoisp23(dcut,cur,isign,ffe,ax,ay,affp,wpm,ci,wf,nx,ny,kstrt,nye,
                kxp,nyh)
   dtimer(dtime,itime,1)
   time = float(dtime)
   tfield = tfield + time
...
# updates cus, modifies cur
   dtimer(dtime,itime,-1)
   isign = 1
   cwppfft2rm3(cus,cur,bs,br,isign,ntpose,mixup,sct,ttp,indx,indy,kstrt,
               nvp,nxeh,nye,kxp,kyp,nypmx,nxhy,nxyh)
   dtimer(dtime,itime,1)
   time = float(dtime)
   tfft[0] = tfft[0] + time
...
# modifies cue
      dtimer(dtime,itime,-1)
      isign = -1
      cwppfft2rm3(cue,cut,bs,br,isign,ntpose,mixup,sct,ttp,indx,indy,
                  kstrt,nvp,nxeh,nye,kxp,kyp,nypmx,nxhy,nxyh)
      dtimer(dtime,itime,1)
      time = float(dtime)
      tfft[0] = tfft[0] + time
...
# with OpenMP: updates dcut, amut, modifies dcu, amu
      dtimer(dtime,itime,-1)
      isign = -1
      cwppfft2rm3(dcu,dcut,bs,br,isign,ntpose,mixup,sct,ttp,indx,indy,
                  kstrt,nvp,nxeh,nye,kxp,kyp,nypmx,nxhy,nxyh)
      tfft[1] = tfft[1] + ttp[0]
      cwppfft2rmn(amu,amut,bs,br,ss,isign,ntpose,mixup,sct,ttp,indx,indy,
                  kstrt,nvp,nxeh,nye,kxp,kyp,nypmx,mdim,nxhy,nxyh)
...
# updates cus, modifies cur
      dtimer(dtime,itime,-1)
      isign = 1
      cwppfft2rm3(cus,cur,bs,br,isign,ntpose,mixup,sct,ttp,indx,indy,
                  kstrt,nvp,nxeh,nye,kxp,kyp,nypmx,nxhy,nxyh)
      dtimer(dtime,itime,1)
      time = float(dtime)
      tfft[0] = tfft[0] + time
...
#  cppgbppush23l(ppart,exyze,bxyze,kpic,noff,nyp,qbme,dt,dt,wke,idimp,
#                nppmx0,nx,ny,mx,my,nxe,nypmx,mx1,mxyp1,ipbc)
# updates ppart, ncl, iholep, ek, irc
   cppgbppushf23l(ppart,exyze,bxyze,kpic,ncl,iholep,noff,nyp,qbme,dt,dt,
