...
! calculate form factor: ffe
      isign = 0
      call CMPPEPOISP23(dcut,cur,isign,ffe,ax,ay,affp,wpm,ci,wf,nx,ny,  &
     &kstrt,nye,kxp,nyh)
!
...
! deposit current with OpenMP: updates cue
      ...
      cue = 0.0
      call CPPGJPPOST2L(ppart,cue,kpic,noff,qme,zero,nppmx0,idimp,nx,ny,&
...
! updates bxyt, wm
      call dtimer(dtime,itime,-1)
      call CMPPBBPOISP23(cut,bxyt,ffc,ci,wm,nx,ny,kstrt,nye,kxp,nyh)
      call dtimer(dtime,itime,1)
      time = real(dtime)
      tfield = tfield + time
!
...
! procedure: updates dcu, amu
      call dtimer(dtime,itime,-1)
      dcu = 0.0; amu = 0.0
      call CPPGDJPPOST2L(ppart,exyze,bxyze,dcu,amu,kpic,noff,nyp,qme,   &
...
! with OpenMP: updates dcut, amut, modifies dcu, amu
      call dtimer(dtime,itime,-1)
      isign = -1
      call CWPPFFT2RM3(dcu,dcut,bs,br,isign,ntpose,mixup,sct,ttp,indx,  &
     &indy,kstrt,nvp,nxeh,nye,kxp,kyp,nypmx,nxhy,nxyh)
      tfft(2) = tfft(2) + ttp
      call CWPPFFT2RMN(amu,amut,bs,br,ss,isign,ntpose,mixup,sct,ttp,indx&
...
! procedure: updates dcut
      call dtimer(dtime,itime,-1)
      call CMPPADCUPERP23(dcut,amut,nx,ny,kstrt,nye,kxp)
      call dtimer(dtime,itime,1)
      time = real(dtime)
      tfield = tfield + time
!
...
! updates cus, modifies cur
      call dtimer(dtime,itime,-1)
      isign = 1
      call CWPPFFT2RM3(cus,cur,bs,br,isign,ntpose,mixup,sct,ttp,indx,   &
     &indy,kstrt,nvp,nxeh,nye,kxp,kyp,nypmx,nxhy,nxyh)
      call dtimer(dtime,itime,1)
      time = real(dtime)
      tfft(1) = tfft(1) + time
...
! with standard procedure: updates cue, dcu, amu
      ...
      call CPPGDCJPPOST2L(ppart,exyze,bxyze,cue,dcu,amu,kpic,noff,nyp,  &
     &qme,qbme,dt,idimp,nppmx0,nx,mx,my,nxe,nypmx,mx1,mxyp1)
...
!     call CPPGBPPUSH23L(ppart,exyze,bxyze,kpic,noff,nyp,qbme,dt,dt,wke,&
!    &idimp,nppmx0,nx,ny,mx,my,nxe,nypmx,mx1,mxyp1,ipbc)
! updates ppart, ncl, iholep, ek, irc
      call CPPGBPPUSHF23L(ppart,exyze,bxyze,kpic,ncl,iholep,noff,nyp,   &
     &qbme,dt,dt,wke,idimp,nppmx0,nx,ny,mx,my,nxe,nypmx,mx1,mxyp1,ntmaxp&
     &,irc)
      call dtimer(dtime,itime,1)
      time = real(dtime)
      tpush = tpush + time
