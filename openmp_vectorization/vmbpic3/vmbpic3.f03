...
! updates ppart, cue
!           call ckncgrjppost3lt(c_loc(ppartt(1,1,1)),                  &
!    &c_loc(cue(1,1,1,1)),c_loc(kpic(1)),qme,dth,ci,nppmx0,idimp,nx,ny, &
!    &nz,mx,my,mz,nxe,nye,nze,mx1,my1,mxyz1,ipbc)
            call cknc2grjppost3lt(c_loc(ppartt(1,1,1)),                 &
     &c_loc(cue(1,1,1,1)),c_loc(kpic(1)),qme,dth,ci,nppmx0,idimp,nx,ny, &
     &nz,mx,my,mz,nxe,nye,nze,mx1,my1,mxyz1,ipbc)
! updates ppart, cue, ncl, ihole, irc-
!           call ckncgrjppostf3lt(c_loc(ppartt(1,1,1)),                 &
!    &c_loc(cue(1,1,1,1)),c_loc(kpic(1)),c_loc(ncl(1,1)),               &
!    &c_loc(ihole(1,1,1)),qme,dth,ci,nppmx0,idimp,nx,ny,nz,mx,my,mz,nxe,&
...
! updates ppart, cue
!           call ckncgjppost3lt(c_loc(ppartt(1,1,1)),c_loc(cue(1,1,1,1))&
!    &,c_loc(kpic(1)),qme,dth,nppmx0,idimp,nx,ny,nz,mx,my,mz,nxe,nye,nze&
!    &,mx1,my1,mxyz1,ipbc)
            call cknc2gjppost3lt(c_loc(ppartt(1,1,1)),                  &
     &c_loc(cue(1,1,1,1)),c_loc(kpic(1)),qme,dth,nppmx0,idimp,nx,ny,nz, &
     &mx,my,mz,nxe,nye,nze,mx1,my1,mxyz1,ipbc)
! updates ppart, cue, ncl, ihole, irc
!           call ckncgjppostf3lt(c_loc(ppartt(1,1,1)),                  &
!    &c_loc(cue(1,1,1,1)),c_loc(kpic(1)),c_loc(ncl(1,1)),               &
!    &c_loc(ihole(1,1,1)),qme,dth,nppmx0,idimp,nx,ny,nz,mx,my,mz,nxe,nye&
...
! KNC function
         ...
         call ckncwfft3rm3(c_loc(cue(1,1,1,1)),isign,c_loc(mixup(1)),   &
     &c_loc(sct(1)),indx,indy,indz,nxeh,nye,nze,nxhyz,nxyzh)
      endif
      call dtimer(dtime,itime,1)
      time = real(dtime)
...
! KNC function
         ...
         call ckncmcuperp3(c_loc(cue(1,1,1,1)),nx,ny,nz,nxeh,nye,nze)
      endif
      call dtimer(dtime,itime,1)
      time = real(dtime)
      tfield = tfield + time
...
! KNC function
            ...
            call ckncmibpois33(c_loc(cue(1,1,1,1)),c_loc(bxyz(1,1,1,1)),&
     &c_loc(ffc(1,1,1)),ci,wm,nx,ny,nz,nxeh,nye,nze,nxh,nyh,nzh)
         endif
         wf = 0.0
         dth = 0.5*dt
      else
...
! KNC function
            ...
            call ckncmmaxwel3(c_loc(exyz(1,1,1,1)),c_loc(bxyz(1,1,1,1)),&
     &c_loc(cue(1,1,1,1)),c_loc(ffc(1,1,1)),ci,dt,wf,wm,nx,ny,nz,nxeh,  &
     &nye,nze,nxh,nyh,nzh)
         endif
      endif
      call dtimer(dtime,itime,1)
...
! KNC function
         ...
         call ckncmemfield3(c_loc(fxyze(1,1,1,1)),c_loc(exyz(1,1,1,1)), &
...
! KNC function
         ...
         call ckncmemfield3(c_loc(bxyze(1,1,1,1)),c_loc(bxyz(1,1,1,1)), &
     &c_loc(ffc(1,1,1)),isign,nx,ny,nz,nxeh,nye,nze,nxh,nyh,nzh)
      endif
      call dtimer(dtime,itime,1)
      time = real(dtime)
...
! KNC function
         ...
         call ckncwfft3rm3(c_loc(bxyze(1,1,1,1)),isign,c_loc(mixup(1)), &
     &c_loc(sct(1)),indx,indy,indz,nxeh,nye,nze,nxhyz,nxyzh)
      endif
      call dtimer(dtime,itime,1)
      time = real(dtime)
...
! updates ppart, wke
            call ckncgrbppush3lt(c_loc(ppartt(1,1,1)),                  &
     &c_loc(fxyze(1,1,1,1)),c_loc(bxyze(1,1,1,1)),c_loc(kpic(1)),qbme,dt&
     &,dth,ci,wke,idimp,nppmx0,nx,ny,nz,mx,my,mz,nxe,nye,nze,mx1,my1,   &
     &mxyz1,ipbc)
! updates ppart, ncl, ihole, wke, irc
!           call ckncgrbppushf3lt(c_loc(ppartt(1,1,1)),                 &
!    &c_loc(fxyze(1,1,1,1)),c_loc(bxyze(1,1,1,1)),c_loc(kpic(1)),       &
!    &c_loc(ncl(1,1)),c_loc(ihole(1,1,1)),qbme,dt,dth,ci,wke,idimp,     &
...
! updates ppart, wke
            call ckncgbppush3lt(c_loc(ppartt(1,1,1)),                   &
     &c_loc(fxyze(1,1,1,1)),c_loc(bxyze(1,1,1,1)),c_loc(kpic(1)),qbme,dt&
     &,dth,wke,idimp,nppmx0,nx,ny,nz,mx,my,mz,nxe,nye,nze,mx1,my1,mxyz1,&
     &ipbc)
! updates ppart, ncl, ihole, wke, irc
!           call ckncgbppushf3lt(c_loc(ppartt(1,1,1)),                  &
!    &c_loc(fxyze(1,1,1,1)),c_loc(bxyze(1,1,1,1)),c_loc(kpic(1)),       &
!    &c_loc(ncl(1,1)),c_loc(ihole(1,1,1)),qbme,dt,dth,wke,idimp,nppmx0, &
