...
void getc4cptr_(unsigned long *carrayref, float complex *carray,
...
/*--------------------------------------------------------------------*/
void avx512_c4allocatex_(unsigned long *sp_c, int *ndim, int *nx,
                         int *ny, int *nz, int *nd, int *irc) {
/* allocate aligned vector 3d complex memory, assign data */
/* pointer to Fortran pointer object sp_c                 */
/* nd = (1,2) = (default,double precision)                */
/* This procedure needs an interface in Fortran90         */
/* interface                                                   */
...
/* local data */
   ...
   avx512_callocate(&cptr,nsize,irc);
/* set reference to C data in real Fortran pointer object */
   getc4cptr_(sp_c,cptr,ndim,nx,ny,nz);
   return;
}
