...
/* set up GPU */
   ...
   init_cl(0,0,&irc);
   if (irc != 0) {
      printf("OpenCL initialization error!\n");
      exit(1);
   }
...
/* g_f4allocate(&g_a,nx,&irc); */
/* g_f4allocate(&g_b,nx,&irc); */
/* g_f4allocate(&g_c,nx,&irc); */
   if (irc != 0) {
...
/* close down GPU */
   end_cl(&irc);
   if (irc != 0) {
      printf("OpenCL termination error!\n");
   }
