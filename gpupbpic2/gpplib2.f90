...
! gpplib2.f90 contains basic communications procedures for 1d partitions
...
! cells in y from remote GPU into vector field
      ...
      integer, dimension(2) :: g_cu, g_cue, g_scs
!     complex dimension g_cu(nxvh,ndim,kypd)
!     real dimension g_cue(ndim,nxe,nypmx)
!     real dimension g_scs(nxe*ndim)
      real, dimension(nxe*ndim) :: scs, scr
      call cgpuppcacguard2xl(g_cu,g_scs,g_cue,nyp,nx,nxe,nypmx,nxvh,kypd&
     &)
      call gpu_fcopyout(scs,g_scs,2*ndim*nxvh)
      call PPPNAGUARD2L(scs,scr,kstrt,nvp,2*ndim*nxvh)
      call gpu_fcopyin(scr,g_scs,2*ndim*nxvh)
      call cgpuppcacguard2yl(g_cu,g_scs,nx,nxvh,kypd)
      end subroutine
!
!-----------------------------------------------------------------------
...
! guard cells in y from remote GPU into extended vector field
      ...
      integer, intent(in) :: nx, nyp, kstrt, nvp, ndim, nxe, nypmx
      integer, intent(in) :: nxvh, kypd
      integer, dimension(2) :: g_fxyz, g_fxyze, g_scs
!     complex dimension g_fxyz(nxvh,ndim,kypd)
!     real dimension g_fxyze(ndim,nxe,nypmx)
!     real dimension g_scs(nxe*ndim)
      real, dimension(nxe*ndim) :: scs, scr
! local data
      integer nnxe
      nnxe = ndim*nxe
      call cgpuppcbguard2xl(g_fxyz,g_scs,g_fxyze,nyp,nx,nxe,nypmx,nxvh, &
     &kypd)
      call gpu_fcopyout(scs,g_scs,2*nxvh*ndim)
      call PPPNCGUARD2L(scs,scr,kstrt,nvp,nnxe)
      call gpu_fcopyin(scr,g_scs,2*nxvh*ndim)
      call cgpuppcbguard2yl(g_fxyze,g_scs,nyp,nx,nxe,nxvh,nypmx)
      end subroutine
!
!-----------------------------------------------------------------------
...
! nxhd must be = nx/2 + 1
      ...
      integer, dimension(2) :: g_f, g_g, g_bsm, g_brm, g_mixup, g_sct
...
! use synchronous communication
               ...
               call gpu_ccopyout(bsm,g_bsm,nxyp)
               call PPPTPOSE(bsm,brm,nxh1,ny,kxpd,kyp,kstrt,nvp)
               call gpu_ccopyin(brm,g_brm,nxyp)
            endif
            call dtimer(dtime,itime,1)
            time = real(dtime)
            ttp(2) = ttp(2) + time
...
! use synchronous communication
               ...
               call gpu_ccopyout(brm,g_brm,nxyp)
               call PPPTPOSE(brm,bsm,ny,nxh1,kyp,kxpd,kstrt,nvp)
               call gpu_ccopyin(bsm,g_bsm,nxyp)
            endif
            call dtimer(dtime,itime,1)
            time = real(dtime)
            ttp(2) = ttp(2) + time
...
! nxhd must be = nx/2 + 1
      ...
      integer, dimension(2) :: g_fn, g_gn, g_bsm, g_brm, g_mixup, g_sct
...
! nxhd must be = nx/2 + 1
      ...
      integer, dimension(2) :: g_f, g_g, g_bsm, g_brm
!     complex dimension g_f(nxhd,kypd), g_g(nyd,kxpd)
!     complex dimension g_bsm(kxpd*kyp,nvp), g_brm(kxpd*kyp,nvp)
      real, dimension(2) :: ttp
      complex, dimension(kxpd*kyp,nvp) :: bsm, brm
...
! nxhd must be = nx/2 + 1
      ...
      integer, dimension(2) :: g_fn, g_gn, g_bsm, g_brm
!     complex dimension g_fn(nxhd,ndim,kypd), g_gn(nyd,ndim,kxpd)
!     complex dimension g_bsm(kxpd*ndim*kyp,nvp)
!     complex dimension g_brm(kxpd*ndim*kyp,nvp)
      real, dimension(2) :: ttp
...
! for distributed data, with 1d domain decomposition in y.
      ...
      integer, dimension(2) :: g_ppart, g_ppbuff, g_sbufl, g_sbufr
      integer, dimension(2) :: g_kpic, g_ncl, g_ihole, g_ncll, g_nclr
      integer, dimension(2) :: g_irc
...
! updates rbufr, rbufl, mcll, mclr
      call dtimer(dtime,itime,-1)
      call gpu_icopyout(ncll,g_ncll,3*mx1)
      call gpu_icopyout(nclr,g_nclr,3*mx1)
      call gpu_fcopyout(sbufl,g_sbufl,idimp*ncll(3,mx1))
      call gpu_fcopyout(sbufr,g_sbufr,idimp*nclr(3,mx1))
      call PPPMOVE2(sbufr,sbufl,rbufr,rbufl,ncll,nclr,mcll,mclr,kstrt,  &
     &nvp,idimp,nbmax,mx1)
      call gpu_icopyin(mcll,g_ncll,3*mx1)
      call gpu_icopyin(mclr,g_nclr,3*mx1)
      call gpu_fcopyin(rbufl,g_sbufl,idimp*mcll(3,mx1))
      call gpu_fcopyin(rbufr,g_sbufr,idimp*mclr(3,mx1))
      call dtimer(dtime,itime,1)
      time = real(dtime)
      ttp(2) = ttp(2) + time
...
! nvp = number of real or virtual processors
      ...
      integer, dimension(2) :: g_bsm, g_btm
...
! better to use a kernel function
      if (nvp==1) then
         call gpu_ccopyout(sm,g_bsm,kxyp)
         do j = 1, kxyp
            tm(j,1) = sm(j,1)
         enddo
         call gpu_ccopyin(tm,g_btm,kxyp)
         return
      endif
      nn = 0
      ns = kxyp
...
! send first group to host from GPU
      call gpu_cascopyout(sm,g_bsm,0,ns,stp)
...
! send next group to host from GPU
            if ((nn+1) < nvp) then
               st = stp + 1
               if (st > 2) st = st - 2 
               call gpu_cascopyout(sm(1,nn+1),g_bsm,ns*nn,ns,st)
            endif
...
! copy received group from host to GPU
               call gpu_cascopyin(tm(1,nn-1),g_btm,ns*(nn-2),ns,3)
            endif
...
! call MPI_WAIT(msid,istatus,ierr); call MPI_WAIT(mrid,istatus,ierr)
      call ACSNDREC(sm,0,0,0,3)
      call gpu_cascopyin(tm(1,nn),g_btm,ns*(nn-1),ns,3)
