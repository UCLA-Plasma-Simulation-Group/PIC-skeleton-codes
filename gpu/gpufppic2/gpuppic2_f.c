...
/* add and copy guard cells with GPU code: updates g_q */
      dtimer(&dtime,&itime,-1);
      fgppcaguard2l(g_q,g_qe,g_scs,scs,scr,nx,nyp,kstrt,nvp,nxe,
                    nypmx,nxhd,kyp);
      dtimer(&dtime,&itime,1);
      time = (float) dtime;
      tguard += time;
...
/* updates g_fxyt, g_we */
      dtimer(&dtime,&itime,-1);
      fgpuppois22t(g_qt,g_fxyt,g_ffct,g_we,nx,ny,kstrt,ny,kxpd,nyh);
      dtimer(&dtime,&itime,1);
      time = (float) dtime;
      tfield += time;
...
/* as well as various buffers */
      isign = 1;
      fwappfft2rcsn(g_fxy,g_fxyt,g_bsm,g_brm,bsm,brm,isign,g_mixup,
                    g_sct,tfft,indx,indy,kstrt,nvp,ndim,kxpd,kyp,nxhd,
                    ny,kyp,nxhy,nxyh);
/* NVIDIA fft */
/*    fgpuppfft2rrcun(g_fxy,g_fxyt,g_bsm,g_brm,bsm,brm,isign,tfft,indx, */
/*                    indy,kstrt,nvp,ndim,kxpd,kyp,nxhd,ny,kyp);        */

/* copy guard cells with GPU code: updates g_fxye */
      dtimer(&dtime,&itime,-1);
      fgppccguard2l(g_fxy,g_fxye,g_scs,scs,scr,nx,nyp,kstrt,nvp,ndim,
                    nxe,nypmx,nxhd,kyp);
      dtimer(&dtime,&itime,1);
      time = (float) dtime;
      tguard += time;
...
/* push particles with GPU code: updates g_ppart, g_wke */
      dtimer(&dtime,&itime,-1);
      fgpuppgppush2l(g_ppart,g_fxye,g_kpic,noff,nyp,qbme,dt,g_wke,nx,ny,
