...
# deposit current with OpenMP: updates part, cue
   ...
   if (relativity==1):
# updates ppart, cue
#     grjppost2l(ppart,cue,kpic,qme,dth,ci,nppmx0,idimp,nx,ny,mx,my,
#                nxe,nye,mx1,mxy1,ipbc)
# updates ppart, cue, ncl, ihole, irc
      grjppostf2l(ppart,cue,kpic,ncl,ihole,qme,dth,ci,nppmx0,idimp,nx,
                  ny,mx,my,nxe,nye,mx1,mxy1,ntmax,irc)
   else:
# updates ppart, cue
#     gjppost2l(ppart,cue,kpic,qme,dth,nppmx0,idimp,nx,ny,mx,my,nxe,
#               nye,mx1,mxy1,ipbc)
# updates ppart, cue, ncl, ihole, irc
      gjppostf2l(ppart,cue,kpic,ncl,ihole,qme,dth,nppmx0,idimp,nx,ny,
   ...
   if (irc[0] != 0):
      if (relativity==1):
         print "grjppostf2l error, irc=", irc[0]
      else:
         print "gjppostf2l error, irc=", irc[0]
      exit(0)
...
# updates ppart, ppbuff, kpic, ncl, and irc
   ...
   tsort = tsort + time
   if (irc[0] != 0):
      print "current pporderf2l error, ntmax, irc=", ntmax, irc[0]
      exit(0)
...
# updates exyz, bxyz, wf, wm
   dtimer(dtime,itime,-1)
   if (ntime==0):
      mibpois23(cue,bxyz,ffc,ci,wm,nx,ny,nxeh,nye,nxh,nyh)
      wf[0] = 0.0
      dth = 0.5*dt
   else:
      mmaxwel2(exyz,bxyz,cue,ffc,ci,dt,wf,wm,nx,ny,nxeh,nye,nxh,nyh)
   dtimer(dtime,itime,1)
   time = float(dtime)
   tfield = tfield + time
...
# updates fxyze
   dtimer(dtime,itime,-1)
   isign = 1
   memfield2(fxyze,exyz,ffc,isign,nx,ny,nxeh,nye,nxh,nyh)
# copy magnetic field with OpenMP: updates bxyze
   isign = -1
   memfield2(bxyze,bxyz,ffc,isign,nx,ny,nxeh,nye,nxh,nyh)
   dtimer(dtime,itime,1)
   time = float(dtime)
   tfield = tfield + time
...
# push particles with OpenMP: updates part, wke
   ...
   if (relativity==1):
# updates ppart, wke
#     grbppush23l(ppart,fxyze,bxyze,kpic,qbme,dt,dth,ci,wke,idimp,
#                 nppmx0,nx,ny,mx,my,nxe,nye,mx1,mxy1,ipbc)
# updates ppart, ncl, ihole, wke, irc
      grbppushf23l(ppart,fxyze,bxyze,kpic,ncl,ihole,qbme,dt,dth,ci,wke,
                   idimp,nppmx0,nx,ny,mx,my,nxe,nye,mx1,mxy1,ntmax,irc)
   else:
# updates ppart, wke
#     gbppush23l(ppart,fxyze,bxyze,kpic,qbme,dt,dth,wke,idimp,nppmx0,
#                nx,ny,mx,my,nxe,nye,mx1,mxy1,ipbc)
# updates ppart, ncl, ihole, wke, irc
      gbppushf23l(ppart,fxyze,bxyze,kpic,ncl,ihole,qbme,dt,dth,wke,
                  idimp,nppmx0,nx,ny,mx,my,nxe,nye,mx1,mxy1,ntmax,irc)
   dtimer(dtime,itime,1)
   time = float(dtime)
   tpush = tpush + time
   if (irc[0] != 0):
      if (relativity==1):
         print "grbppushf23l error, irc=", irc[0]
      else:
