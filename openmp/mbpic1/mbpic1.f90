...
! updates ppart, cue
!        call GRJPPOST1L(ppart,cue,kpic,qme,dth,ci,nppmx0,idimp,nx,mx,  &
!    &nxe,mx1,ipbc)
! updates ppart, cue, ncl, ihole, irc
         call GRJPPOSTF1L(ppart,cue,kpic,ncl,ihole,qme,dth,ci,nppmx0,   &
     &idimp,nx,mx,nxe,mx1,ntmax,irc)
      else
! updates ppart, cue
!        call GJPPOST1L(ppart,cue,kpic,qme,dth,nppmx0,idimp,nx,mx,nxe,  &
!    &mx1,ipbc)
! updates ppart, cue, ncl, ihole, irc
         call GJPPOSTF1L(ppart,cue,kpic,ncl,ihole,qme,dth,nppmx0,idimp, &
     &nx,mx,nxe,mx1,ntmax,irc)
      ...
      if (irc /= 0) then
         if (relativity==1) then
            write (*,*) 'GRJPPOSTF1L error: irc=', irc
         else
            write (*,*) 'GJPPOSTF1L error: irc=', irc
         endif
         stop
      endif
!
...
! push particles with OpenMP:
      ...
      if (relativity==1) then
! updates ppart, wke
!        call GRBPPUSH13L(ppart,fxyze,byze,kpic,omx,qbme,dt,dth,ci,wke, &
!    &idimp,nppmx0,nx,mx,nxe,mx1,ipbc)
! updates ppart, ncl, ihole, wke, irc
         call GRBPPUSHF13L(ppart,fxyze,byze,kpic,ncl,ihole,omx,qbme,dt, &
     &dth,ci,wke,idimp,nppmx0,nx,mx,nxe,mx1,ntmax,irc)
      else
! updates ppart, wke
!        call GBPPUSH13L(ppart,fxyze,byze,kpic,omx,qbme,dt,dth,wke,idimp&
!    &,nppmx0,nx,mx,nxe,mx1,ipbc)
! updates ppart, ncl, ihole, wke, irc
         call GBPPUSHF13L(ppart,fxyze,byze,kpic,ncl,ihole,omx,qbme,dt,  &
     &dth,wke,idimp,nppmx0,nx,mx,nxe,mx1,ntmax,irc)
      endif
      call dtimer(dtime,itime,1)
      time = real(dtime)
      tpush = tpush + time
      if (irc /= 0) then
         if (relativity==1) then
            write (*,*) 'GRBPPUSHF13L error: irc=', irc
         else
