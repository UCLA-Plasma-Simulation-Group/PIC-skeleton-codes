...
! updates ppart, cue
!        call GRJPPOST3L(ppart,cue,kpic,qme,dth,ci,nppmx0,idimp,nx,ny,nz&
!    &,mx,my,mz,nxe,nye,nze,mx1,my1,mxyz1,ipbc)
! updates ppart, cue, ncl, ihole, irc
         call GRJPPOSTF3L(ppart,cue,kpic,ncl,ihole,qme,dth,ci,nppmx0,   &
     &idimp,nx,ny,nz,mx,my,mz,nxe,nye,nze,mx1,my1,mxyz1,ntmax,irc)
      else
! updates ppart, cue
!        call GJPPOST3L(ppart,cue,kpic,qme,dth,nppmx0,idimp,nx,ny,nz,mx,&
!    &my,mz,nxe,nye,nze,mx1,my1,mxyz1,ipbc)
! updates ppart, cue, ncl, ihole, irc
         call GJPPOSTF3L(ppart,cue,kpic,ncl,ihole,qme,dth,nppmx0,idimp, &
     &nx,ny,nz,mx,my,mz,nxe,nye,nze,mx1,my1,mxyz1,ntmax,irc)
      ...
      if (irc /= 0) then
         if (relativity==1) then
            write (*,*) 'GRJPPOSTF3L error: irc=', irc
         else
            write (*,*) 'GJPPOSTF3L error: irc=', irc
         endif
         stop
      endif
!
...
! updates exyz, bxyz, wf, wm
      call dtimer(dtime,itime,-1)
      if (ntime==0) then
         call MIBPOIS33(cue,bxyz,ffc,ci,wm,nx,ny,nz,nxeh,nye,nze,nxh,nyh&
     &,nzh)
         wf = 0.0
         dth = 0.5*dt
      else
         call MMAXWEL3(exyz,bxyz,cue,ffc,ci,dt,wf,wm,nx,ny,nz,nxeh,nye, &
     &nze,nxh,nyh,nzh)
      endif
      call dtimer(dtime,itime,1)
      time = real(dtime)
...
! updates fxyze
      call dtimer(dtime,itime,-1)
      isign = 1
      call MEMFIELD3(fxyze,exyz,ffc,isign,nx,ny,nz,nxeh,nye,nze,nxh,nyh,&
     &nzh)
! copy magnetic field with OpenMP: updates bxyze
      isign = -1
      call MEMFIELD3(bxyze,bxyz,ffc,isign,nx,ny,nz,nxeh,nye,nze,nxh,nyh,&
     &nzh)
      call dtimer(dtime,itime,1)
      time = real(dtime)
      tfield = tfield + time
...
! push particles with OpenMP: updates part, wke
      ...
      if (relativity==1) then
! updates ppart, wke
!        call GRBPPUSH3L(ppart,fxyze,bxyze,kpic,qbme,dt,dth,ci,wke,idimp&
!    &,nppmx0,nx,ny,nz,mx,my,mz,nxe,nye,nze,mx1,my1,mxyz1,ipbc)
! updates ppart, ncl, ihole, wke, irc
         call GRBPPUSHF3L(ppart,fxyze,bxyze,kpic,ncl,ihole,qbme,dt,dth, &
     &ci,wke,idimp,nppmx0,nx,ny,nz,mx,my,mz,nxe,nye,nze,mx1,my1,mxyz1,  &
     &ntmax,irc)
      else
! updates ppart, wke
!        call GBPPUSH3L(ppart,fxyze,bxyze,kpic,qbme,dt,dth,wke,idimp,   &
!    &nppmx0,nx,ny,nz,mx,my,mz,nxe,nye,nze,mx1,my1,mxyz1,ipbc)
! updates ppart, ncl, ihole, wke, irc
         call GBPPUSHF3L(ppart,fxyze,bxyze,kpic,ncl,ihole,qbme,dt,dth,  &
     ...
     &,irc)
      ...
      if (irc /= 0) then
         if (relativity==1) then
            write (*,*) 'GRBPPUSHF3L error: irc=', irc
         else
