...
isign = 0
cmpois22(qe,fxye,isign,ffc,ax,ay,affp,we,nx,ny,nxeh,nye,nxh,nyh)
...
# transform force to real space with OpenMP: updates fxye
   dtimer(dtime,itime,-1)
   isign = 1
   cwfft2rm2(fxye,isign,mixup,sct,indx,indy,nxeh,nye,nxhy,nxyh)
   dtimer(dtime,itime,1)
   time = float(dtime)
   tfft = tfft + time
...
# push particles with OpenMP: updates part, wke
   ...
   dtimer(dtime,itime,-1)
# updates ppart, wke
#  cgppush2l(ppart,fxye,kpic,qbme,dt,wke,idimp,nppmx0,nx,ny,mx,my,nxe,
#            nye,mx1,mxy1,ipbc)
# updates ppart, ncl, ihole, wke, irc
   cgppushf2l(ppart,fxye,kpic,ncl,ihole,qbme,dt,wke,idimp,nppmx0,nx,ny,
