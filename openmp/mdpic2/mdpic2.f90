...
! calculate form factor: ffe
      isign = 0
      call MEPOIS23(dcu,cus,isign,ffe,ax,ay,affp,wpm,ci,wf,nx,ny,nxeh,  &
     &nye,nxh,nyh)
!
...
! deposit current with OpenMP: updates cue
      ...
      cue = 0.0
      call GJPPOST2L(ppart,cue,kpic,qme,zero,nppmx0,idimp,nx,ny,mx,my,  &
...
! updates bxyze, wm
      call dtimer(dtime,itime,-1)
      call MBBPOIS23(cue,bxyze,ffc,ci,wm,nx,ny,nxeh,nye,nxh,nyh)
      call dtimer(dtime,itime,1)
      time = real(dtime)
      tfield = tfield + time
!
...
! updates dcu, amu
      call dtimer(dtime,itime,-1)
      dcu = 0.0; amu = 0.0
      call GDJPPOST2L(ppart,exyze,bxyze,dcu,amu,kpic,qme,qbme,dt,idimp, &
...
! with OpenMP: updates dcu, amu
      call dtimer(dtime,itime,-1)
      isign = -1
      call WFFT2RM3(dcu,isign,mixup,sct,indx,indy,nxeh,nye,nxhy,nxyh)
      call WFFT2RMN(amu,ss,isign,mixup,sct,indx,indy,nxeh,nye,mdim,nxhy,&
     &nxyh)
      call dtimer(dtime,itime,1)
      time = real(dtime)
      tfft = tfft + time
...
! updates dcu
      call dtimer(dtime,itime,-1)
      call MADCUPERP23(dcu,amu,nx,ny,nxeh,nye)
      call dtimer(dtime,itime,1)
      time = real(dtime)
      tfield = tfield + time
!
...
! updates cus
      call dtimer(dtime,itime,-1)
      isign = 1
      call WFFT2RM3(cus,isign,mixup,sct,indx,indy,nxeh,nye,nxhy,nxyh)
      call dtimer(dtime,itime,1)
      time = real(dtime)
      tfft = tfft + time
!
...
! with OpenMP: updates cue, dcu, amu
      ...
      call GDCJPPOST2L(ppart,exyze,bxyze,cue,dcu,amu,kpic,qme,qbme,dt,  &  
...
! push particles with OpenMP:
      ...
      call dtimer(dtime,itime,-1)
! updates ppart, wke
!     call GBPPUSH23L(ppart,exyze,bxyze,kpic,qbme,dt,dt,wke,idimp,nppmx0&
!    &,nx,ny,mx,my,nxe,nye,mx1,mxy1,ipbc)
! updates ppart, ncl, ihole, wke, irc
      call GBPPUSHF23L(ppart,exyze,bxyze,kpic,ncl,ihole,qbme,dt,dt,wke, &
