c Fortran library for Skeleton 2-1/2D Darwin PIC Code field diagnostics
...
c-----------------------------------------------------------------------
      subroutine MAPOTP23(cu,axy,ffc,ci,wm,nx,ny,nxvh,nyv,nxhd,nyhd)
...
c input: cu,ffc,ci,nx,ny,nxvh,nyv,nxhd,nyhd, output: axy,wm
c approximate flop count is: 68*nxc*nyc + 21*(nxc + nyc)
c where nxc = nx/2 - 1, nyc = ny/2 - 1
c vector potential is calculated using the equation:
c ax(kx,ky) = ci*ci*g(kx,ky)*cux(kx,ky)
c ay(kx,ky) = ci*ci*g(kx,ky)*cuy(kx,ky)
c az(kx,ky) = ci*ci*g(kx,ky)*cuz(kx,ky)
c where kx = 2pi*j/nx, ky = 2pi*k/ny, and j,k = fourier mode numbers,
c g(kx,ky) = (affp/(kx**2+ky**2))*s(kx,ky),
c s(kx,ky) = exp(-((kx*ax)**2+(ky*ay)**2)/2), except for
c ax(kx=pi) = ay(kx=pi) = az(kx=pi) = 0 
c ax(ky=pi) = ay(ky=pi) = az(ky=pi) = 0
c and ax(kx=0,ky=0) = ay(kx=0,ky=0) = az(kx=0,ky=0) = 0.
c cu(i,j,k) = complex current density for fourier mode (j-1,k-1)
c axy(1,j,k) = x component of complex vector potential
c axy(2,j,k) = y component of complex vector potential
c axy(3,j,k) = z component of complex vector potential
...
c nyhd = second dimension of form factor array, must be >= nyh
      ...
      real ci, wm
      complex cu, axy, ffc
      dimension cu(3,nxvh,nyv), axy(3,nxvh,nyv)
      dimension ffc(nxhd,nyhd)
c local data
      integer nxh, nyh, ny2, j, k, k1
      real dnx, dny, ci2, at1, at2
      complex zero
      double precision wp, sum1
      nxh = nx/2
      nyh = max(1,ny/2)
...
!$OMP PARALLEL DO PRIVATE(j,k,k1,at1,at2,wp) REDUCTION(+:sum1)
      ...
      at1 = at2*aimag(ffc(j,k))
      axy(1,j,k) = at2*cu(1,j,k)
      axy(2,j,k) = at2*cu(2,j,k)
      axy(3,j,k) = at2*cu(3,j,k)
      axy(1,j,k1) = at2*cu(1,j,k1)
      axy(2,j,k1) = at2*cu(2,j,k1)
      axy(3,j,k1) = at2*cu(3,j,k1)
...
c mode numbers kx = 0, nx/2
      ...
      at1 = at2*aimag(ffc(1,k))
      axy(1,1,k) = at2*cu(1,1,k)
      axy(2,1,k) = at2*cu(2,1,k)
      axy(3,1,k) = at2*cu(3,1,k)
      axy(1,1,k1) = zero
      axy(2,1,k1) = zero
      axy(3,1,k1) = zero
...
c mode numbers ky = 0, ny/2
      ...
      at1 = at2*aimag(ffc(j,1))
      axy(1,j,1) = at2*cu(1,j,1)
      axy(2,j,1) = at2*cu(2,j,1)
      axy(3,j,1) = at2*cu(3,j,1)
      axy(1,j,k1) = zero
      axy(2,j,k1) = zero
      axy(3,j,k1) = zero
