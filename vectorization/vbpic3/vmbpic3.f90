...
!
      integer :: i, j, k, l
      real :: eps, epsmax, wks
      real, dimension(:,:,:), pointer :: pparts
      real, dimension(:,:,:,:), pointer :: gue
!
      irc = 0
...
!
      call avx512_f3allocate(pparts,nppmx0,idimp,mxyz1,irc)
      call avx512_f4allocate(gue,ndim,nxe,nye,nze,irc)
!
      nloop = 20
!
...
!
      ...
      do i = 1, idimp
      do j = 1, nppmx0
      pparts(j,i,l) = ppartt(j,i,l)
      enddo
      enddo
      enddo
      call SET_VZERO3(gue,mx,my,mz,ndim,nxe,nye,nze,mx1,my1,mxyz1)
!
...
! updates ppart, cue
!        call GRJPPOST3LT(ppartt,cue,kpic,qme,dth,ci,nppmx0,idimp,nx,ny,&
!    &nz,mx,my,mz,nxe,nye,nze,mx1,my1,mxyz1,ipbc)
         call VGRJPPOST3LT(ppartt,cue,kpic,qme,dth,ci,nppmx0,idimp,nx,ny&
     &,nz,mx,my,mz,nxe,nye,nze,mx1,my1,mxyz1,ipbc)
! updates ppart, cue, ncl, ihole, irc
!        call GRJPPOSTF3LT(ppartt,cue,kpic,ncl,ihole,qme,dth,ci,nppmx0, &
!    &idimp,nx,ny,nz,mx,my,mz,nxe,nye,nze,mx1,my1,mxyz1,ntmax,irc)
!        call VGRJPPOSTF3LT(ppartt,cue,kpic,ncl,ihole,qme,dth,ci,nppmx0,&
!    &idimp,nx,ny,nz,mx,my,mz,nxe,nye,nze,mx1,my1,mxyz1,ntmax,irc)
      else
! updates ppart, cue
!        call GJPPOST3LT(ppartt,cue,kpic,qme,dth,nppmx0,idimp,nx,ny,nz, &
!    &mx,my,mz,nxe,nye,nze,mx1,my1,mxyz1,ipbc)
         call VGJPPOST3LT(ppartt,cue,kpic,qme,dth,nppmx0,idimp,nx,ny,nz,&
     &mx,my,mz,nxe,nye,nze,mx1,my1,mxyz1,ipbc)
! updates ppart, cue, ncl, ihole, irc
!        call GJPPOSTF3LT(ppartt,cue,kpic,ncl,ihole,qme,dth,nppmx0,idimp&
...
!
!     call ckncgjppost3lt(pparts,gue,kpic,qme,dth,nppmx0,idimp,nx,ny,nz,&
!    &mx,my,mz,nxe,nye,nze,mx1,my1,mxyz1,ipbc)
!-----------------------------------------------------------------------
      call ckncgrjppost3lt(pparts,gue,kpic,qme,dth,ci,nppmx0,idimp,nx,ny&
     &,nz,mx,my,mz,nxe,nye,nze,mx1,my1,mxyz1,ipbc)
!
      epsmax = 0.0
      do l = 1, nze
      ...
      do j = 1, nxe
      do i = 1, ndim
      eps = abs(cue(i,j,k,l)-gue(i,j,k,l))
      if (eps > epsmax) then
         write (72,*) i,j,k,l,cue(i,j,k,l),gue(i,j,k,l),eps
         epsmax = eps
      endif
      enddo
      enddo
      enddo
      enddo
      write (72,*) ntime,'cue epsmax=',epsmax
      write (*,*) ntime,'cue epsmax=',epsmax
!
      epsmax = 0.0
      do l = 1, mxyz1
      do i = 1, idimp
      do j = 1, nppmx0
      eps = abs(ppartt(j,i,l)-pparts(j,i,l))
      if (eps > epsmax) then
         write (71,*) j,i,l,ppartt(j,i,l),pparts(j,i,l),eps
         epsmax = eps
      endif
      enddo
      enddo
      enddo
      write (71,*) 'ppartt epsmax=',epsmax
      write (*,*) 'ppartt epsmax=',epsmax
!
...
! updates exyz, bxyz, wf, wm
     ...
     &nyh,nzh)
         ...
         call VMMAXWEL3(exyz,bxyz,cue,ffc,ci,dt,wf,wm,nx,ny,nz,nxeh,nye,&
     &nze,nxh,nyh,nzh)
      endif
      call dtimer(dtime,itime,1)
      time = real(dtime)
...
! updates fxyze
      call dtimer(dtime,itime,-1)
      isign = 1
      call VMEMFIELD3(fxyze,exyz,ffc,isign,nx,ny,nz,nxeh,nye,nze,nxh,nyh&
     &,nzh)
! copy magnetic field with OpenMP: updates bxyze
      isign = -1
      call VMEMFIELD3(bxyze,bxyz,ffc,isign,nx,ny,nz,nxeh,nye,nze,nxh,nyh&
     &,nzh)
      call dtimer(dtime,itime,1)
      time = real(dtime)
      tfield = tfield + time
