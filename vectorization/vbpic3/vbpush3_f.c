...
void gjpost3lt_(float *part, float *cu, float *qm, float *dt, int *nop,
                int *npe, int *idimp, int *nx, int *ny, int *nz,
                int *nxv, int *nyv, int *nzv, int *ipbc);

void grjpost3lt_(float *part, float *cu, float *qm, float *dt,
                 float *ci, int *nop, int *npe, int *idimp, int *nx,
                 int *ny, int *nz, int *nxv, int *nyv, int *nzv,
                 int *ipbc);

void vgjpost3lt_(float *part, float *cu, float *qm, float *dt, int *nop,
                 int *npe, int *idimp, int *nx, int *ny, int *nz,
                 int *nxv, int *nyv, int *nzv, int *ipbc);

void vgrjpost3lt_(float *part, float *cu, float *qm, float *dt,
                  float *ci, int *nop, int *npe, int *idimp, int *nx,
                  int *ny, int *nz, int *nxv, int *nyv, int *nzv,
                  int *ipbc);
...
void vibpois33_(float complex *cu, float complex *bxyz,
                float complex *ffc, float *ci, float *wm, int *nx,
                int *ny, int *nz, int *nxvh, int *nyv, int *nzv,
                int *nxhd, int *nyhd, int *nzhd);

void vmaxwel3_(float complex *exyz, float complex *bxyz,
...
void vemfield3_(float complex *fxyz, float complex *exyz,
...
void cgbpush3lt(float part[], float fxyz[], float bxyz[], float qbm,
                ...
                int nzv, int ipbc) {
   gbpush3lt_(part,fxyz,bxyz,&qbm,&dt,&dtc,ek,&idimp,&nop,&npe,&nx,&ny,
              &nz,&nxv,&nyv,&nzv,&ipbc);
   return;
}
...
void cgrbpush3lt(float part[], float fxyz[], float bxyz[], float qbm,
                 ...
                 int nyv, int nzv, int ipbc) {
   grbpush3lt_(part,fxyz,bxyz,&qbm,&dt,&dtc,&ci,ek,&idimp,&nop,&npe,&nx,
...
void cvgbpush3lt(float part[], float fxyz[], float bxyz[], float qbm,
                 ...
                 int npe, int nx, int ny, int nz, int nxv, int nyv,
                 int nzv, int ipbc) {
   vgbpush3lt_(part,fxyz,bxyz,&qbm,&dt,&dtc,ek,&idimp,&nop,&npe,&nx,&ny,
               &nz,&nxv,&nyv,&nzv,&ipbc);
   return;
}
...
void cvgrbpush3lt(float part[], float fxyz[], float bxyz[], float qbm,
                  ...
                  int nop, int npe, int nx, int ny, int nz, int nxv,
                  int nyv, int nzv, int ipbc) {
   vgrbpush3lt_(part,fxyz,bxyz,&qbm,&dt,&dtc,&ci,ek,&idimp,&nop,&npe,
...
void cgjpost3lt(float part[], float cu[], float qm, float dt, int nop,
                ...
                int nyv, int nzv, int ipbc) {
   gjpost3lt_(part,cu,&qm,&dt,&nop,&npe,&idimp,&nx,&ny,&nz,&nxv,&nyv,
              &nzv,&ipbc);
   return;
}
...
void cgrjpost3lt(float part[], float cu[], float qm, float dt, float ci,
                 ...
                 int nxv, int nyv, int nzv, int ipbc) {
   grjpost3lt_(part,cu,&qm,&dt,&ci,&nop,&npe,&idimp,&nx,&ny,&nz,&nxv,
               &nyv,&nzv,&ipbc);
   return;
}
...
void cvgjpost3lt(float part[], float cu[], float qm, float dt, int nop,
                 int npe,int idimp, int nx, int ny, int nz, int nxv,
                 int nyv, int nzv, int ipbc) {
   vgjpost3lt_(part,cu,&qm,&dt,&nop,&npe,&idimp,&nx,&ny,&nz,&nxv,&nyv,
               &nzv,&ipbc);
   return;
}
...
void cvgrjpost3lt(float part[], float cu[], float qm, float dt,
                  ...
                  int nz, int nxv, int nyv, int nzv, int ipbc) {
   vgrjpost3lt_(part,cu,&qm,&dt,&ci,&nop,&npe,&idimp,&nx,&ny,&nz,&nxv,
                &nyv,&nzv,&ipbc);
   return;
}
...
void cvibpois33(float complex cu[], float complex bxyz[],
                ...
                int nyhd, int nzhd) {
   vibpois33_(cu,bxyz,ffc,&ci,wm,&nx,&ny,&nz,&nxvh,&nyv,&nzv,&nxhd,
              &nyhd,&nzhd);
   return;
}
...
void cvmaxwel3(float complex exyz[], float complex bxyz[],
               ...
               int nxvh, int nyv, int nzv, int nxhd, int nyhd,
               int nzhd) {
   vmaxwel3_(exyz,bxyz,cu,ffc,&ci,&dt,wf,wm,&nx,&ny,&nz,&nxvh,&nyv,&nzv,
             &nxhd,&nyhd,&nzhd);
   return;
}
...
void cvemfield3(float complex fxyz[], float complex exyz[],
                ...
                int nxvh, int nyv, int nzv, int nxhd, int nyhd,
                int nzhd) {
   vemfield3_(fxyz,exyz,ffc,&isign,&nx,&ny,&nz,&nxvh,&nyv,&nzv,&nxhd,
              &nyhd,&nzhd);
   return;
}
