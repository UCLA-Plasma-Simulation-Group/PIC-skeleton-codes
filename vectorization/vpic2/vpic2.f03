...
! SSE2 function
         ...
         call csse2pois22(c_loc(qe(1,1)),c_loc(fxye(1,1,1)),isign,      &
...
! transform force to real space with standard procedure: updates fxye
      ...
      isign = 1
      if (kvec==1) then
         call WFFT2RV2(fxye,isign,mixup,sct,indx,indy,nxeh,nye,nxhy,    &
     &nxyh)
! SSE2 function
      else if (kvec==2) then
         call csse2wfft2r2(c_loc(fxye(1,1,1)),isign,c_loc(mixup(1)),    &
     &c_loc(sct(1)),indx,indy,nxeh,nye,nxhy,nxyh)
      endif
      call dtimer(dtime,itime,1)
      time = real(dtime)
...
! SSE2 function
         ...
         call csse2gpush2lt(c_loc(partt(1,1)),c_loc(fxye(1,1,1)),qbme,dt&
     &,c_loc(wke),idimp,np,npe,nx,ny,nxe,nye,ipbc)
      endif
      call dtimer(dtime,itime,1)
      time = real(dtime)
      tpush = tpush + time
