...
! kvec = (1,2) = run (autovector,AVX512) version
      integer :: kvec = 1
...
! calculate form factors
      isign = 0
      call cvpois33(qe,fxyze,isign,ffc,ax,ay,az,affp,we,nx,ny,nz,nxeh,  &
     &nye,nze,nxh,nyh,nzh)
! initialize electrons
      call cdistr3t(partt,vtx,vty,vtz,vx0,vy0,vz0,npx,npy,npz,idimp,npe,&
     &nx,ny,nz,ipbc)
!
...
! deposit charge with standard procedure: updates qe
      ...
      qe = 0.0
!     call cgpost3lt(partt,qe,qme,np,npe,idimp,nxe,nye,nze)
      call cvgpost3lt(partt,qe,qme,np,npe,idimp,nxe,nye,nze)
!     call cv2gpost3lt(partt,qe,qme,np,npe,idimp,nxe,nye,nze)
      call dtimer(dtime,itime,1)
      time = real(dtime)
      tdpost = tdpost + time
!
...
! transform charge to fourier space with standard procedure: updates qe
      call dtimer(dtime,itime,-1)
      isign = -1
      call cwfft3rvx(qe,isign,mixup,sct,indx,indy,indz,nxeh,nye,nze,    &
     &nxhyz,nxyzh)
      call dtimer(dtime,itime,1)
      time = real(dtime)
      tfft = tfft + time
...
! transform force to real space with standard procedure: updates fxyze
      call dtimer(dtime,itime,-1)
      isign = 1
      call cwfft3rv3(fxyze,isign,mixup,sct,indx,indy,indz,nxeh,nye,nze, &
     &nxhyz,nxyzh)
      call dtimer(dtime,itime,1)
      time = real(dtime)
      tfft = tfft + time
...
! push particles with standard procedure: updates part, wke
      wke = 0.0
      call dtimer(dtime,itime,-1)
!     call cgpush3lt(partt,fxyze,qbme,dt,wke,idimp,np,npe,nx,ny,nz,nxe, &
!    &nye,nze,ipbc)
      call cvgpush3lt(partt,fxyze,qbme,dt,wke,idimp,np,npe,nx,ny,nz,nxe,&
     &nye,nze,ipbc)
!     call cv2gpush3lt(partt,fxyze,qbme,dt,wke,idimp,np,npe,nx,ny,nz,nxe&
!    &,nye,nze,ipbc)
      call dtimer(dtime,itime,1)
      time = real(dtime)
      tpush = tpush + time
...
! sort particles by cell for standard procedure
      if (sortime > 0) then
         if (mod(ntime,sortime)==0) then
            call dtimer(dtime,itime,-1)
            call cdsortp3yzlt(partt,partt2,npic,idimp,np,npe,ny1,nyz1)
! exchange pointers
            tpartt => partt
            partt => partt2
            partt2 => tpartt
