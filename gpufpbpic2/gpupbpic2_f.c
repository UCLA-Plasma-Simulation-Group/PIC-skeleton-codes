...
/* prepare NVIDIA ffts */
   fgpupfft2rrcuinit(nx,kypp,ndim);
   fgpupfft2cuinit(kxpp,ny,ndim);
...
/* copy in solver arrays to GPU */
   ...
   fgpu_ccopyin(ffct,g_ffct,nyh*kxpd);
...
/* initialize transverse electromagnetic fields */
   for (j = 0; j < ny*ndim*kxpd; j++) {
      fxyzt[j] = 0.0 + 0.0*_Complex_I;
   }
   fgpu_ccopyin(fxyzt,g_exyzt,ny*ndim*kxpd);
...
/* copy to GPU */
   ...
   fgpu_fcopyin(ppart,g_ppart,nppmx0*idimp*mxyp1);
   fgpu_icopyin(kpic,g_kpic,mxyp1);
   fgpu_zfmem(g_we,kxpd);
   fgpu_zfmem(g_wf,kxpd);
   fgpu_zfmem(g_wm,kxpd);
...
/* deposit current with GPU code: updates g_ppart, g_cue */
      ...
      fgpu_zfmem(g_cue,ndim*nxe*nypmx);
      if (relativity==1) {
         fgpu2pprjppost2l(g_ppart,g_cue,g_kpic,noff,qme,dth,ci,nppmx0,
                          idimp,nx,ny,mx,my,nxe,nypmx,mx1,mxyp1,ipbc);
      }
      else {
         fgpu2ppjppost2l(g_ppart,g_cue,g_kpic,noff,qme,dth,nppmx0,
                         idimp,nx,ny,mx,my,nxe,nypmx,mx1,mxyp1,ipbc);
      }
      dtimer(&dtime,&itime,1);
      time = (float) dtime;
...
/* as well as various buffers */
      fgpporder2l(g_ppart,g_ppbuff,g_sbufl,g_sbufr,g_kpic,g_ncl,g_ihole,
...
/* deposit charge with GPU code: updates g_qe */
      dtimer(&dtime,&itime,-1);
      fgpu_zfmem(g_qe,nxe*nypmx);
      fgpu2ppgppost2l(g_ppart,g_qe,g_kpic,noff,qme,idimp,nppmx0,mx,my,
                      nxe,nypmx,mx1,mxyp1);
      dtimer(&dtime,&itime,1);
      time = (float) dtime;
      tdpost += time;
...
/* add and copy guard cells with GPU code: updates g_cu, g_q */
      dtimer(&dtime,&itime,-1);
      fgppcacguard2l(g_cu,g_cue,g_scs,scs,scr,nx,nyp,kstrt,nvp,ndim,
                     nxe,nypmx,nxhd,kyp);
      fgppcaguard2l(g_q,g_qe,g_scs,scs,scr,nx,nyp,kstrt,nvp,nxe,nypmx,
                    nxhd,kyp);
      dtimer(&dtime,&itime,1);
      time = (float) dtime;
      tguard += time;
...
/* as well as various buffers */
      isign = -1;
      fwappfft2rcs(g_q,g_qt,g_bsm,g_brm,bsm,brm,isign,g_mixup,g_sct,
                   tfft,indx,indy,kstrt,nvp,kxpd,kyp,nxhd,ny,kyp,nxhy,
                   nxyh);
/* NVIDIA fft */
/*    fgpuppfft2rrcu(g_q,g_qt,g_bsm,g_brm,bsm,brm,isign,tfft,indx,indy, */
...
/* updates g_cu, g_cut, as well as various buffers   */
      isign = -1;
      fwappfft2rcsn(g_cu,g_cut,g_bsm,g_brm,bsm,brm,isign,g_mixup,g_sct,
                    tfft,indx,indy,kstrt,nvp,ndim,kxpd,kyp,nxhd,ny,kyp,
                    nxhy,nxyh);
/* NVIDIA fft */
/*    fgpuppfft2rrcun(g_cu,g_cut,g_bsm,g_brm,bsm,brm,isign,tfft,indx, */
/*                    indy,kstrt,nvp,ndim,kxpd,kyp,nxhd,ny,kyp);      */

/* take transverse part of current with GPU code: updates g_cut */
      dtimer(&dtime,&itime,-1);
      fgpuppcuperp2t(g_cut,nx,ny,kstrt,ny,kxpd);
      dtimer(&dtime,&itime,1);
      time = (float) dtime;
      tfield += time;
...
/* updates g_exyzt, g_bxyzt, g_wf, g_wm                             */
      dtimer(&dtime,&itime,-1);
      if (ntime==0) {
         fgpuippbpoisp23t(g_cut,g_bxyzt,g_ffct,ci,g_wm,nx,ny,kstrt,ny,
                          kxpd,nyh);
         wf = 0.0;
         dth = 0.5*dt;
      }
      else {
         fgpuppmaxwel2t(g_exyzt,g_bxyzt,g_cut,g_ffct,affp,ci,dt,g_wf,
...
/* updates g_fxyzt, g_we                                  */
      dtimer(&dtime,&itime,-1);
      fgpuppois23t(g_qt,g_fxyzt,g_ffct,g_we,nx,ny,kstrt,ny,kxpd,nyh);
      dtimer(&dtime,&itime,1);
      time = (float) dtime;
      tfield += time;
...
/* updates g_fxyzt                                                     */
      dtimer(&dtime,&itime,-1);
      isign = 1;
      fgpuppemfield2t(g_fxyzt,g_exyzt,g_ffct,isign,nx,ny,kstrt,ny,kxpd,
                      nyh);
/* copy magnetic field with GPU code: updates g_hxyzt */
      isign = -1;
      fgpuppemfield2t(g_hxyzt,g_bxyzt,g_ffct,isign,nx,ny,kstrt,ny,kxpd,
                      nyh);
      dtimer(&dtime,&itime,1);
      time = (float) dtime;
      tfield += time;
...
/* as well as various buffers */
      isign = 1;
      fwappfft2rcsn(g_fxyz,g_fxyzt,g_bsm,g_brm,bsm,brm,isign,g_mixup,
                    g_sct,tfft,indx,indy,kstrt,nvp,ndim,kxpd,kyp,nxhd,
                    ny,kyp,nxhy,nxyh);
/* NVIDIA fft */
/*    fgpuppfft2rrcun(g_fxyz,g_fxyzt,g_bsm,g_brm,bsm,brm,isign,tfft,  */
...
/* as well as various buffers */
      isign = 1;
      fwappfft2rcsn(g_hxyz,g_hxyzt,g_bsm,g_brm,bsm,brm,isign,g_mixup,
                    g_sct,tfft,indx,indy,kstrt,nvp,ndim,kxpd,kyp,nxhd,
                    ny,kyp,nxhy,nxyh);
/* NVIDIA fft */
/*    fgpuppfft2rrcun(g_hxyz,g_hxyzt,g_bsm,g_brm,bsm,brm,isign,tfft,  */
/*                    indx,indy,kstrt,nvp,ndim,kxpd,kyp,nxhd,ny,kyp); */

/* copy guard cells with GPU code: updates g_fxyze, g_bxyze */
      dtimer(&dtime,&itime,-1);
      fgppcbguard2l(g_fxyz,g_fxyze,g_scs,scs,scr,nx,nyp,kstrt,nvp,ndim,
                    nxe,nypmx,nxhd,kyp);
      fgppcbguard2l(g_hxyz,g_bxyze,g_scs,scs,scr,nx,nyp,kstrt,nvp,ndim,
                    nxe,nypmx,nxhd,kyp);
      dtimer(&dtime,&itime,1);
      time = (float) dtime;
      tguard += time;
...
/* push particles with GPU code: updates g_ppart, g_wke */
      dtimer(&dtime,&itime,-1);
      if (relativity==1) {
         fgpuppgrbppush23l(g_ppart,g_fxyze,g_bxyze,g_kpic,noff,nyp,qbme,
                           dt,dth,ci,g_wke,idimp,nppmx0,nx,ny,mx,my,nxe,
                           nypmx,mx1,mxyp1,ipbc);
      }
      else {
         fgpuppgbppush23l(g_ppart,g_fxyze,g_bxyze,g_kpic,noff,nyp,qbme,
...
/* energy diagnostic */
      if (ntime==0) {
         fgpu_zfmem(g_sum,1);
         fgpusum2(g_we,g_sum,kxpd);
         fgpu_fcopyout(&we,g_sum,1);
         fgpu_zfmem(g_sum,1);
         fgpusum2(g_wf,g_sum,kxpd);
         fgpu_fcopyout(&wf,g_sum,1);
         fgpu_zfmem(g_sum,1);
         fgpusum2(g_wm,g_sum,kxpd);
         fgpu_fcopyout(&wm,g_sum,1);
         fgpu_zfmem(g_sum,1);
         fgpusum2(g_wke,g_sum,mxyp1);
