...
c-----------------------------------------------------------------------
      subroutine PPOIS23T(qt,fxyt,isign,ffct,ax,ay,affp,we,nx,ny,kstrt, 
     1nyv,kxp1,nyhd)
...
c vector length is second dimension.  Zeros out z component.
c for isign = 0, input: isign,ax,ay,affp,nx,ny,kstrt,nyv,kxp1,nyhd,
c output: ffct
c for isign /= 0, input: qt,ffct,isign,nx,ny,kstrt,nyv,kxp1,nyhd,
c output: fxyt,we
...
c fxyt(k,3,j) = zero,
c for fourier mode (jj-1,k-1), where jj = j + kxp1*(kstrt - 1)
...
c real(ffct(k,j)) = potential green's function g1*(kstrt - 1)
...
c nyhd = first dimension of form factor array, must be >= nyh
      ...
      dimension qt(nyv,kxp1), fxyt(nyv,3,kxp1)
