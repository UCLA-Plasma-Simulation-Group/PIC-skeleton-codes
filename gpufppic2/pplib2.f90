...
! PPPCNCGUARD2L sends/receives guard cells in y for complex scalar array
!               linear interpolation, and distributed data with
!               non-uniform partition.  for copying guard cells.
! PPPCNAGUARD2L sends/receives guard cells in y for complex scalar array
!               linear interpolation, and distributed data with
!               non-uniform partition.  for adding guard cells.
...
!
      ...
      public :: PPPCNCGUARD2L, PPPCNAGUARD2L
...
! this subroutine sends/receives guard cells in y for complex scalar
! array.
...
! kstrt = starting data block number
! nvp = number of real or virtual processors
! nxvh = size of complex array to send
! linear interpolation, for distributed data
      implicit none
      integer, intent(in) :: kstrt, nvp, nxvh
...
!-----------------------------------------------------------------------
      subroutine PPPCNCGUARD2L(scs,scr,kstrt,nvp,nxvh)
      use pplib2, only: SUB => PPPCNCGUARD2L
      implicit none
      integer :: kstrt, nvp, nxvh
      complex, dimension(nxvh) :: scs, scr
      call SUB(scs,scr,kstrt,nvp,nxvh)
      end subroutine
!
!-----------------------------------------------------------------------
      subroutine PPPCNAGUARD2L(scs,scr,kstrt,nvp,nxvh)
      use pplib2, only: SUB => PPPCNAGUARD2L
      implicit none
      integer:: kstrt, nvp, nxvh
      complex, dimension(nxvh) :: scs, scr
      call SUB(scs,scr,kstrt,nvp,nxvh)
