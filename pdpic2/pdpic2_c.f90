...
! calculate form factor: ffe
      isign = 0
      call CPPEPOISP23(dcut,cur,isign,ffe,ax,ay,affp,wpm,ci,wf,nx,ny,   &
     &kstrt,nye,kxp,nyh)
!
...
! procedure: updates cus, modifies cur
      call dtimer(dtime,itime,-1)
      isign = 1
      call CWPPFFT2R3(cus,cur,bs,br,isign,ntpose,mixup,sct,ttp,indx,indy&
     &,kstrt,nvp,nxeh,nye,kxp,kyp,nypmx,nxhy,nxyh)
      call dtimer(dtime,itime,1)
      time = real(dtime)
      tfft(1) = tfft(1) + time
