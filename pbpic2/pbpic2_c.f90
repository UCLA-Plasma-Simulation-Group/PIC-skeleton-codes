...
! deposit current with standard procedure: updates part, cue, ihole
      ...
      if (relativity==1) then
         call CPPGRJPOST2L(part,cue,edges,npp,noff,ihole,qme,dth,ci,nx, &
     &ny,idimp,npmax,nxe,nypmx,idps,ntmax,ipbc)
      else
         call CPPGJPOST2L(part,cue,edges,npp,noff,ihole,qme,dth,nx,ny,  &
...
! procedure: updates exyz, bxyz
      call dtimer(dtime,itime,-1)
      if (ntime==0) then
         call CIPPBPOISP23(cut,bxyz,ffc,ci,wm,nx,ny,kstrt,nye,kxp,nyh)
         wf = 0.0
         dth = 0.5*dt
      else
         call CPPMAXWEL2(exyz,bxyz,cut,ffc,affp,ci,dt,wf,wm,nx,ny,kstrt,&
     &nye,kxp,nyh)
      endif
      call dtimer(dtime,itime,1)
      time = real(dtime)
...
! procedure: updates fxyt
      call dtimer(dtime,itime,-1)
      isign = 1
      call CPPEMFIELD2(fxyt,exyz,ffc,isign,nx,ny,kstrt,nye,kxp,nyh)
! copy magnetic field with standard procedure: updates bxyt
      isign = -1
      call CPPEMFIELD2(bxyt,bxyz,ffc,isign,nx,ny,kstrt,nye,kxp,nyh)
      call dtimer(dtime,itime,1)
      time = real(dtime)
      tfield = tfield + time
!
...
! push particles: updates part, wke, and ihole
      ...
      wke = 0.0
      if (relativity==1) then
         call CPPGRBPUSH23L(part,fxyze,bxyze,edges,npp,noff,ihole,qbme, &
     &dt,dth,ci,wke,nx,ny,idimp,npmax,nxe,nypmx,idps,ntmax,ipbc)
      else
         call CPPGBPUSH23L(part,fxyze,bxyze,edges,npp,noff,ihole,qbme,dt&
