c Fortran library for Skeleton 2-1/2D Darwin MPI PIC Code field
c diagnostics
...
c-----------------------------------------------------------------------
      subroutine PPAPOTP23(cu,axy,ffc,ci,wm,nx,ny,kstrt,nyv,kxp,nyhd)
...
c input: cu,ffc,ci,nx,ny,kstrt,nyv,kxp,nyhd, output: axy,wm
...
c aimag(ffc(k,j)) = finite-size particle shape factor s
c real(ffc(k,j)) = potential green's function g
c for fourier mode (jj-1,k-1), where jj = j + kxp*(kstart - 1)
