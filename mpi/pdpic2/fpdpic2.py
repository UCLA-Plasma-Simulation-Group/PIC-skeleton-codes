...
isign = 0
ppepoisp23(dcut,exyt,isign,ffe,ax,ay,affp,wpm,ci,wf,nx,ny,kstrt,nye,kxp,
           nyh)
...
# updates exyt, wf
   dtimer(dtime,itime,-1)
   isign = -1
   ppepoisp23(dcut,exyt,isign,ffe,ax,ay,affp,wpm,ci,wf,nx,ny,kstrt,nye,
              kxp,nyh)
   dtimer(dtime,itime,1)
   time = float(dtime)
   tfield = tfield + time
...
# procedure: updates cus, modifies exyt
   dtimer(dtime,itime,-1)
   isign = 1-
   wppfft2r3(cus,exyt,bs,br,isign,ntpose,mixup,sct,ttp,indx,indy,kstrt,
             nvp,nxeh,nye,kxp,kyp,nypmx,nxhy,nxyh)
   dtimer(dtime,itime,1)
   time = float(dtime)
   tfft[0] = tfft[0] + time
...
# updates exyt, wf
      dtimer(dtime,itime,-1)
      isign = -1
      ppepoisp23(dcut,exyt,isign,ffe,ax,ay,affp,wpm,ci,wf,nx,ny,kstrt,
                 nye,kxp,nyh)
      dtimer(dtime,itime,1)
      time = float(dtime)
      tfield = tfield + time
...
# procedure: updates cus, modifies exyt
      dtimer(dtime,itime,-1)
      isign = 1
      wppfft2r3(cus,exyt,bs,br,isign,ntpose,mixup,sct,ttp,indx,indy,
                kstrt,nvp,nxeh,nye,kxp,kyp,nypmx,nxhy,nxyh)
      dtimer(dtime,itime,1)
      time = float(dtime)
      tfft[0] = tfft[0] + time
