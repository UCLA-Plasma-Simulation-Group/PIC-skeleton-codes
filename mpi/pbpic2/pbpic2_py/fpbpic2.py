...
# deposit current with standard procedure: updates part, cue, ihole
   ...
   cue.fill(0.0)
   if (relativity==1):
      ppgrjpost2l(part,cue,edges,npp,noff,ihole,qme,dth,ci,nx,ny,idimp,
                  npmax,nxe,nypmx,idps,ntmax,ipbc)
   else:
      ppgjpost2l(part,cue,edges,npp,noff,ihole,qme,dth,nx,ny,idimp,
                 npmax,nxe,nypmx,idps,ntmax,ipbc)
   dtimer(dtime,itime,1)
   time = float(dtime)
...
# check for particle manager error
   if (info[0] != 0):
      ierr[0] = info[0]
      if (kstrt==1):
         print "current particle manager error: ierr=", ierr[0]
      ppexit()
      exit(0)
...
# procedure: updates exyz, bxyz, wf, wm
   dtimer(dtime,itime,-1)
   if (ntime==0):
      ippbpoisp23(cut,bxyz,ffc,ci,wm,nx,ny,kstrt,nye,kxp,nyh)
      wf[0] = 0.0
      dth = 0.5*dt
   else:
      ppmaxwel2(exyz,bxyz,cut,ffc,affp,ci,dt,wf,wm,nx,ny,kstrt,nye,kxp,
                nyh)
   dtimer(dtime,itime,1)
   time = float(dtime)
   tfield = tfield + time
...
# procedure: updates fxyt
   dtimer(dtime,itime,-1)
   isign = 1
   ppemfield2(fxyt,exyz,ffc,isign,nx,ny,kstrt,nye,kxp,nyh)
# copy magnetic field with standard procedure: updates bxyt
   isign = -1
   ppemfield2(bxyt,bxyz,ffc,isign,nx,ny,kstrt,nye,kxp,nyh)
   dtimer(dtime,itime,1)
   time = float(dtime)
   tfield = tfield + time
...
# push particles: updates part, wke, and ihole
   ...
   dtimer(dtime,itime,-1)
   if (relativity==1):
      ppgrbpush23l(part,fxyze,bxyze,edges,npp,noff,ihole,qbme,dt,dth,ci,
                   wke,nx,ny,idimp,npmax,nxe,nypmx,idps,ntmax,ipbc)
   else:
      ppgbpush23l(part,fxyze,bxyze,edges,npp,noff,ihole,qbme,dt,dth,wke,
                  nx,ny,idimp,npmax,nxe,nypmx,idps,ntmax,ipbc)
   dtimer(dtime,itime,1)
   time = float(dtime)
   tpush = tpush + time
# check for ihole overflow error
