c Fortran Library for Skeleton 3D Darwin MPI PIC Code
...
c-----------------------------------------------------------------------
      subroutine PPGMJPOST32L(part,amu,npp,noff,qm,idimp,npmax,nxv,nypmx
     1,nzpmx,idds)
...
c part(3,n) = position z of particle n at t in partition
c part(4,n) = x velocity of particle n at t - dt/2 in partition
c part(5,n) = y velocity of particle n at t - dt/2 in partition
c part(6,n) = z velocity of particle n at t - dt/2 in partition
c amu(i,j,k,l) = ith component of momentum flux at grid point j,kk,ll
c where kk = k + noff(1) - 1, and ll = l + noff(2) - 1
c qm = charge on particle, in units of e
c npp = number of particles in partition
c noff(1) = lowermost global gridpoint in y in particle partition
...
c idds = dimensionality of domain decomposition
      ...
      integer noff
      real part, amu
      dimension part(idimp,npmax), amu(6,nxv,nypmx,nzpmx)
      dimension noff(idds)
c local data
      integer j, mnoff, lnoff, nn, mm, ll, np, mp, lp
...
c-----------------------------------------------------------------------
      subroutine PPGDJPOST32L(part,fxyz,bxyz,npp,noff,dcu,amu,qm,qbm,dt,
     1idimp,npmax,nxv,nypmx,nzpmx,idds)
...
c part(2,n) = position y of particle n at t in partition
c part(3,n) = position z of particle n at t in partition
c part(4,n) = velocity vx of particle n at t - dt/2 in partition
c part(5,n) = velocity vy of particle n at t - dt/2 in partition
c part(6,n) = velocity vz of particle n at t - dt/2 in partition
c fxyz(1,j,k,l) = x component of force/charge at grid (j,kk,ll)
c fxyz(2,j,k,l) = y component of force/charge at grid (j,kk,ll)
c fxyz(3,j,k,l) = z component of force/charge at grid (j,kk,ll)
...
c-----------------------------------------------------------------------
      subroutine PPGDCJPOST32L(part,fxyz,bxyz,npp,noff,cu,dcu,amu,qm,qbm
     1,dt,idimp,npmax,nxv,nypmx,nzpmx,idds)
...
c interpolation for distributed data with 2D spatial decomposition
c scalar version using guard cells
...
c-----------------------------------------------------------------------
      subroutine PPBBPOISP332(cu,bxyz,ffc,ci,wm,nx,ny,nz,kstrt,nvpy,nvpz
...
c-----------------------------------------------------------------------
      subroutine PPDCUPERP32(dcu,amu,nx,ny,nz,kstrt,nvpy,nvpz,nzv,kxyp, 
     1kyzp)
...
c-----------------------------------------------------------------------
      subroutine PPADCUPERP32(dcu,amu,nx,ny,nz,kstrt,nvpy,nvpz,nzv,kxyp,
     1kyzp)
c this subroutine calculates transverse part of the derivative of
...
c-----------------------------------------------------------------------
      subroutine PPEPOISP332(dcu,exyz,isign,ffe,ax,ay,az,affp,wp0,ci,wf,
     1nx,ny,nz,kstrt,nvpy,nvpz,nzv,kxyp,kyzp,nzhd)
c this subroutine solves 3d poisson's equation in fourier space for
...
c mode numbers kx = 0, nx/2
  ...
  160 continue
      wf = real(nx)*real(ny)*real(nz)*wp/affp
      return
...
c with packed data, parallelized with MPI
      implicit none
      integer isign, ntpose, indx, indy, indz, kstrt, nvpy, nvpz
      integer nxvh, nyv, nzv, kxyp, kyp, kyzp, kzp
...
c inverse fourier transform
      if (isign.lt.0) then
c perform x fft
         call PPFFT32RNXX(f,ss,isign,mixup,sct,indx,indy,indz,kstrt,nvp,
     1kypi,kypp,nxvh,kzpp,kypd,kzpd,ndim,nxhyzd,nxyzhd)
c transpose f array to g
         call PWTIMERA(-1,ttp,dtime)
     ...
     1,nxvh,nyv,kxypd,kypd,kzpd)
         call PWTIMERA(1,ttp,dtime)
c perform y fft
         call PPFFT32RNXY(g,isign,mixup,sct,indx,indy,indz,kstrt,nvpy,  
...
c transpose g array to h
     ...
     1nvpz,ndim,nyv,nzv,kxypd,kyzpd,kzpd)
         call PWTIMERA(1,tp,dtime)
c perform z fft
         call PPFFT32RNXZ(h,isign,mixup,sct,indx,indy,indz,kstrt,nvpy,  
...
c kzpp = number of z indices used
c kxypd, kzpd  = third and fourth dimension of g
c mixup = array of bit reversed addresses
c sct = sine/cosine table
c nxhyzd = maximum of (nx/2,ny,nz)
c nxyzhd = one half of maximum of (nx,ny,nz)
...
c kxypi = initial x index used
c kxypp = number of x indices used
c nzv = second dimension of h
c kyzpp = number of y indices used
c kxypd, kyzp = third and fourth dimension of h
c mixup = array of bit reversed addresses
c sct = sine/cosine table
c nxhyzd = maximum of (nx/2,ny,nz)
c nxyzhd = one half of maximum of (nx,ny,nz)
...
c-----------------------------------------------------------------------
      subroutine PPFFT32RNXX(f,ss,isign,mixup,sct,indx,indy,indz,kstrt, 
...
c complex fast fourier transforms and their inverses for a subset of
c y and z, using complex arithmetic, where N = ndim
c for data which is distributed in blocks, with 2D spatial decomposition
c for isign = (-1,1), input: all, output: f
...
c inverse fourier transform
      ani = 0.5/(real(nx)*real(ny)*real(nz))
c swap complex components
      call PPSWAPC32N(f,ss,isign,nxh,kypi,kypt,nxvh,kzpp,kypd,kzpd,ndim)
      do 140 n = 1, kzpp
      nrx = nxhyz/nxh
c bit-reverse array elements in x
      do 30 j = 1, nxh
...
c-----------------------------------------------------------------------
      subroutine PPFFT32RNXY(g,isign,mixup,sct,indx,indy,indz,kstrt,nvpy
     1,nvpz,kxypi,kxypp,nyv,kzpp,kxypd,kzpd,ndim,nxhyzd,nxyzhd)
c this subroutine performs the y part of N three dimensional real to
c complex fast fourier transforms and their inverses for a subset of
c x and z, using complex arithmetic, where N = ndim
c for data which is distributed in blocks, with 2D spatial decomposition
c for isign = (-1,1), input: all, output: g
c for isign = -1, approximate flop count: M*(5*log2(N) + 19/2)/nvp
c for isign = 1,  approximate flop count: M*(5*log2(N) + 15/2)/nvp
c where M = (nx/2)*ny*nz, and nvp = number of procs
c indx/indy/indz = exponent which determines length in x/y/z direction,
c where nx=2**indx, ny=2**indy, nz=2**indz
c if isign = -1, an inverse fourier transform is performed
...
c-----------------------------------------------------------------------
      subroutine PPFFT32RNXZ(h,isign,mixup,sct,indx,indy,indz,kstrt,nvpy
     1,nvpz,kxypi,kxypp,nzv,kyzp,kxypd,kyzpd,ndim,nxhyzd,nxyzhd)
c this subroutine performs the z part of N three dimensional real to
c complex fast fourier transforms and their inverses for a subset of
c x and y, using complex arithmetic, where N = ndim
c for data which is distributed in blocks, with 2D spatial decomposition
c for isign = (-1,1), input: all, output: h
c for isign = -1, approximate flop count: M*(5*log2(N) + 19/2)/nvp
c for isign = 1,  approximate flop count: M*(5*log2(N) + 15/2)/nvp
...
c scramble modes kx = 0, nx/2
  ...
  190 do 280 n = 1, kyzpp
      nrz = nxhyz/nz
c bit-reverse array elements in z
      do 220 l = 1, nz
      l1 = (mixup(l) - 1)/nrz + 1
      if (l.ge.l1) go to 220
      do 210 i = kxypi, kxypt
      do 200 jj = 1, ndim
      t1 = h(jj,l1,i,n)
      h(jj,l1,i,n) = h(jj,l,i,n)
      h(jj,l,i,n) = t1
...
c-----------------------------------------------------------------------
      subroutine PPSWAPC32N(f,s,isign,nxh,kypi,kypt,nxvh,kzpp,kypd,kzpd,
     1ndim)
...
c ndim = leading dimension of array f
      ...
      real f, s
      dimension f(ndim,2*nxvh,kypd,kzpd), s(2*ndim*nxvh)
c local data
