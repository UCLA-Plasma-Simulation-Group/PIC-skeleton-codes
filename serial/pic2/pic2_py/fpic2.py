...
isign = 0
pois22(qe,fxye,isign,ffc,ax,ay,affp,we,nx,ny,nxeh,nye,nxh,nyh)
...
# transform force to real space with standard procedure: updates fxye
   dtimer(dtime,itime,-1)
   isign = 1
   wfft2r2(fxye,isign,mixup,sct,indx,indy,nxeh,nye,nxhy,nxyh)
   dtimer(dtime,itime,1)
   time = float(dtime)
   tfft = tfft + time
...
# push particles with standard procedure: updates part, wke
   wke[0] = 0.0
   dtimer(dtime,itime,-1)
   gpush2l(part,fxye,qbme,dt,wke,idimp,np,nx,ny,nxe,nye,ipbc)
   dtimer(dtime,itime,1)
   time = float(dtime)
   tpush = tpush + time
