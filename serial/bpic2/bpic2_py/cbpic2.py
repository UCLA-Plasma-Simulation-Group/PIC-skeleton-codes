...
# procedure: updates exyz, bxyz, wf, wm
   dtimer(dtime,itime,-1)
   if (ntime==0):
      cibpois23(cue,bxyz,ffc,ci,wm,nx,ny,nxeh,nye,nxh,nyh)
      wf[0] = 0.0
      dth = 0.5*dt
   else:
      cmaxwel2(exyz,bxyz,cue,ffc,ci,dt,wf,wm,nx,ny,nxeh,nye,nxh,nyh)
   dtimer(dtime,itime,1)
   time = float(dtime)
   tfield = tfield + time
...
# procedure: updates fxyze
   dtimer(dtime,itime,-1)
   isign = 1
   cemfield2(fxyze,exyz,ffc,isign,nx,ny,nxeh,nye,nxh,nyh)
# copy magnetic field with standard procedure: updates bxyze
   isign = -1
   cemfield2(bxyze,bxyz,ffc,isign,nx,ny,nxeh,nye,nxh,nyh)
   dtimer(dtime,itime,1)
   time = float(dtime)
   tfield = tfield + time
