...
idimp = 5; ipbc = 1; sortime = 50; relativity = 1
...
# deposit current with standard procedure: updates part, cue
   ...
   cue.fill(0.0)
   if (relativity==1):
      grjpost2l(part,cue,qme,dth,ci,np,idimp,nx,ny,nxe,nye,ipbc)
   else:
      gjpost2l(part,cue,qme,dth,np,idimp,nx,ny,nxe,nye,ipbc)
   dtimer(dtime,itime,1)
   time = float(dtime)
...
# procedure: updates exyz, bxyz, wf, wm
   dtimer(dtime,itime,-1)
   if (ntime==0):
      ibpois23(cue,bxyz,ffc,ci,wm,nx,ny,nxeh,nye,nxh,nyh)
      wf[0] = 0.0
      dth = 0.5*dt
   else:
      maxwel2(exyz,bxyz,cue,ffc,ci,dt,wf,wm,nx,ny,nxeh,nye,nxh,nyh)
   dtimer(dtime,itime,1)
   time = float(dtime)
   tfield = tfield + time
...
# procedure: updates fxyze
   dtimer(dtime,itime,-1)
   isign = 1
   emfield2(fxyze,exyz,ffc,isign,nx,ny,nxeh,nye,nxh,nyh)
# copy magnetic field with standard procedure: updates bxyze
   isign = -1
   emfield2(bxyze,bxyz,ffc,isign,nx,ny,nxeh,nye,nxh,nyh)
   dtimer(dtime,itime,1)
   time = float(dtime)
   tfield = tfield + time
...
# push particles with standard procedure: updates part, wke
   ...
   dtimer(dtime,itime,-1)
   if (relativity==1):
      grbpush23l(part,fxyze,bxyze,qbme,dt,dth,ci,wke,idimp,np,nx,ny,nxe,
                 nye,ipbc)
   else:
      gbpush23l(part,fxyze,bxyze,qbme,dt,dth,wke,idimp,np,nx,ny,nxe,nye,
                ipbc)
   dtimer(dtime,itime,1)
   time = float(dtime)
   tpush = tpush + time
