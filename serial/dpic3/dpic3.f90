...
! deposit current with standard procedure: updates cue
      ...
      cue = 0.0
      call GJPOST3L(part,cue,qme,zero,np,idimp,nx,ny,nz,nxe,nye,nze,ipbc&
     &)
!     call GSJPOST3L(part,cue,qme,zero,np,idimp,nx,ny,nz,nxe,nye,       &
!    &nxe*nye*nze,ipbc)
      call dtimer(dtime,itime,1)
      time = real(dtime)
...
! updates bxyze, wm
      call dtimer(dtime,itime,-1)
      call BBPOIS33(cue,bxyze,ffc,ci,wm,nx,ny,nz,nxeh,nye,nze,nxh,nyh,  &
     &nzh)
      call dtimer(dtime,itime,1)
      time = real(dtime)
      tfield = tfield + time
...
! procedure: updates dcu, amu
      call dtimer(dtime,itime,-1)
      dcu = 0.0; amu = 0.0
      call GDJPOST3L(part,exyze,bxyze,dcu,amu,qme,qbme,dt,idimp,np,nxe, &
     &nye,nze)
...
! with standard procedure: updates dcu, amu
      call dtimer(dtime,itime,-1)
      isign = -1
      call WFFT3R3(dcu,isign,mixup,sct,indx,indy,indz,nxeh,nye,nze,nxhyz&
     &,nxyzh)
      call WFFT3RN(amu,ss,isign,mixup,sct,indx,indy,indz,nxeh,nye,nze,  &
     &mdim,nxhyz,nxyzh)
      call dtimer(dtime,itime,1)
      time = real(dtime)
      tfft = tfft + time
...
! procedure: updates dcu
      call dtimer(dtime,itime,-1)
      call ADCUPERP3(dcu,amu,nx,ny,nz,nxeh,nye,nze)
      call dtimer(dtime,itime,1)
      time = real(dtime)
      tfield = tfield + time
!
...
! procedure: updates cus
      call dtimer(dtime,itime,-1)
      isign = 1
      call WFFT3R3(cus,isign,mixup,sct,indx,indy,indz,nxeh,nye,nze,nxhyz&
     &,nxyzh)
      call dtimer(dtime,itime,1)
      time = real(dtime)
      tfft = tfft + time
...
! with standard procedure: updates cue, dcu, amu
      ...
      call GDCJPOST3L(part,exyze,bxyze,cue,dcu,amu,qme,qbme,dt,idimp,np,&
     &nxe,nye,nze)
...
! push particles with standard procedure: updates part, wke
      wke = 0.0
      call dtimer(dtime,itime,-1)
      call GBPUSH3L(part,exyze,bxyze,qbme,dt,dt,wke,idimp,np,nx,ny,nz,  &
     &nxe,nye,nze,ipbc)
!     call GSBPUSH3L(part,exyze,bxyze,qbme,dt,dt,wke,idimp,np,nx,ny,nz, &
!    &nxe,nye,nxe*nye*nze,ipbc)
      call dtimer(dtime,itime,1)
      time = real(dtime)
      tpush = tpush + time
!
