c 3d PIC library for Skeleton 3D Electromagnetic PIC Code field
c diagnostics
...
c input: q,ffc,isign,nx,ny,nz,nxv,nyv,nyhd,nzhd, output: pot,we
...
c-----------------------------------------------------------------------
      subroutine AVRPOT33(axyz,bxyz,ffc,ci,nx,ny,nz,nxvh,nyv,nzv,nxhd,ny
     1hd,nzhd)
c this subroutine solves 3d poisson's equation in fourier space for
...
c nxvh = first dimension of input array pot, nxe >= nx/2
c nyv = second dimension of input array pot, nye >= ny
c nzv = third dimension of input array pot, nze >= nz
...
c modesx/modesy/modesz = number of modes to store in x/y/z direction,
c where modesx <= nx/2+1, modesy <= ny/2+1, modesz <= nz/2+1
c nxvh = first dimension of output array pot, nxe >= nx/2
c nyv = second dimension of output array pot, nye >= ny
c nzv = third dimension of output array pot, nze >= nz
...
c nxvh = second dimension of input array vpot, nxeh >= nx/2
c nyv = third dimension of input array vpot, nye >= ny
c nzv = fourth dimension of input array vpot, nze >= nz
...
c-----------------------------------------------------------------------
     ...
     1,nxvh,nyv,nzv,modesxd,modesyd,modeszd)
c this subroutine extracts lowest order modes from a location in an
c unpacked complex vector array bpott and stores them into a packed
c complex vector array bpot
c modes stored: kx=(0,1,...,NX/2), ky=(0,+-1,+-2,...,+-(NY/2-1),NY/2),
c kz=(0,+-1,+-2,...,+-(NZ/2-1),NZ/2)
c nx/ny/nz = system length in x/y/z direction
c modesx/modesy/modesz = number of modes to store in x/y/z direction,
