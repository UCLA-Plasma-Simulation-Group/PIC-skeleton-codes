...
! deposit current with standard procedure: updates part, cue
      ...
      if (relativity==1) then
         call CGRJPOST3L(part,cue,qme,dth,ci,np,idimp,nx,ny,nz,nxe,nye, &
     &nze,ipbc)
      else
         call CGJPOST3L(part,cue,qme,dth,np,idimp,nx,ny,nz,nxe,nye,nze, &
     &ipbc)
      endif
      call dtimer(dtime,itime,1)
      time = real(dtime)
...
! procedure: updates exyz, bxyz, wf, wm
      call dtimer(dtime,itime,-1)
      if (ntime==0) then
         call CIBPOIS33(cue,bxyz,ffc,ci,wm,nx,ny,nz,nxeh,nye,nze,nxh,nyh&
     &,nzh)
         wf = 0.0
         dth = 0.5*dt
      else
         call CMAXWEL3(exyz,bxyz,cue,ffc,ci,dt,wf,wm,nx,ny,nz,nxeh,nye, &
     &nze,nxh,nyh,nzh)
      endif
      call dtimer(dtime,itime,1)
      time = real(dtime)
...
! procedure: updates fxyze
      call dtimer(dtime,itime,-1)
      isign = 1
      call CEMFIELD3(fxyze,exyz,ffc,isign,nx,ny,nz,nxeh,nye,nze,nxh,nyh,&
     &nzh)
! copy magnetic field with standard procedure: updates bxyze
      isign = -1
      call CEMFIELD3(bxyze,bxyz,ffc,isign,nx,ny,nz,nxeh,nye,nze,nxh,nyh,&
     &nzh)
      call dtimer(dtime,itime,1)
      time = real(dtime)
      tfield = tfield + time
...
! push particles with standard procedure: updates part, wke
      ...
      call dtimer(dtime,itime,-1)
      if (relativity==1) then
         call CGRBPUSH3L(part,fxyze,bxyze,qbme,dt,dth,ci,wke,idimp,np,nx&
     &,ny,nz,nxe,nye,nze,ipbc)
      else
         call CGBPUSH3L(part,fxyze,bxyze,qbme,dt,dth,wke,idimp,np,nx,ny,&
     &nz,nxe,nye,nze,ipbc)
      endif
      call dtimer(dtime,itime,1)
      time = real(dtime)
